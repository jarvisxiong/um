...
public class PdCache {
	 ...
	 * @param keyValue
	 * @param refField
	 * @return 如果返回值为Null，则把查询值返回
	 */
	public static String getRefString(Class entityClass, String keyField, String keyValue, String refField) {
		String value=keyValue;
		try {
			value = getValueString(getRefObject(entityClass, keyField, keyValue, refField));
		} catch (Exception e) {
...
//		return value;
	 ...
	 * @return
	 */
	public static Object getRefObject(Class entityClass, String keyField, String keyValue, String refField) {
		Object obj = getValueObject(getRefEntity(entityClass, keyField, keyValue), refField);
		return obj;
	}
	 ...
	 * @return
	 */
	public static Object getRefEntity(Class entityClass, String keyField, String keyValue) {
		if (StringUtils.isEmpty(keyValue))
			return null;
		HashMap<String, Object> mapKeyValue = getKeyValueMap(entityClass, keyField);
		if (mapKeyValue == null)
			return null;
	 ...
	 * @return
	 */
	public static Collection<String> getFieldList(Class entityClass, String keyField) {
		HashMap<String, Object> mapKeyValue = getKeyValueMap(entityClass, keyField);
	...
	public static List getEntityList(Class entityClass, Map<String, Object> map, List<String> lstProp) {
	...
	public static List getEntityList(Class entityClass, boolean forceLoad, Map<String, Object> map) {
	...
	public static List getList(Class entityClass, String keyField, String condField, Object condValue) {
		HashMap<String, Object> mapKeyValue = getKeyValueMap(entityClass, keyField);
		return PowerUtils.setCollection2List(mapKeyValue.values(), condField, condValue);

	}

	private static List getEntityList(Class entityClass, boolean forceLoad, Map<String, Object> map, List<String> lstProp) {
		List lstEntity = null;
		if (!forceLoad) {
	...
	private static Object doNullException(String entityClassName, String keyField, String keyValue) {
		try {
	...
	private static void addSearchTime(String entityClassName, String keyField, String keyValue) {
		HashMap<String, HashMap<String, Integer>> mapFieldValue = mapNull.get(entityClassName);
		if (mapFieldValue == null || mapFieldValue.isEmpty()) {
			...
			if (mapKeyTime == null || mapKeyTime.isEmpty()) {
				...
				mapKeyTime.put(keyValue, mapKeyTime.get(keyValue) == null ? 0 : mapKeyTime.get(keyValue) + 1);

			}
		}
	}

	private static int getSearchTime(String entityClassName, String keyField, String keyValue) {
		HashMap<String, HashMap<String, Integer>> mapFieldValue = mapNull.get(entityClassName);
	 ...
	 * @return
	 */
	private static HashMap<String, Object> getKeyValueMap(Class entityClass, String keyField) {
		HashMap<String, Object> mapKeyValue;
		HashMap<String, HashMap<String, Object>> mapField = mapOODB.get(entityClass.getName());
		if (mapField == null) {
	 ...
	 * @return
	 */
	private static HashMap<String, Object> setKeyValueMap(Class entityClass, String keyField) {
		try {
			...
			fillKeyValueMap(mapField, entityClass.getName(), lstEntity, keyField);
	...
	public static void fillKeyValueMap(HashMap<String, HashMap<String, Object>> mapField, String entityClassName, List lst, String keyField) {
		...
		if (isExistProperty(entityClassName, replaceCodeName(keyField))) {
			HashMap<String, Object> mapKeyOppositeValue = fillKeyValueMap(lst, replaceCodeName(keyField));
			mapField.put(replaceCodeName(keyField), mapKeyOppositeValue);
		}
	}

	private static HashMap<String, Object> fillKeyValueMap(List lst, String keyField) {
		try {
			HashMap<String, Object> mapKeyValue = new HashMap<String, Object>();
			for (int i = 0; i < lst.size(); i++) {
				mapKeyValue.put(BeanUtils.getProperty(lst.get(i), keyField), lst.get(i));
			}
			return mapKeyValue;
		} catch (Exception e) {
			return null;
	 ...
	 * @return
	 */
	private static boolean isExistProperty(String entityName, String propertyName) {
		if (propertyName == null)
			return false;
		try {
			return PropertyUtils.isWriteable(Class.forName(entityName).newInstance(), propertyName);
		} catch (Exception e) {
			return false;
		}
	}
