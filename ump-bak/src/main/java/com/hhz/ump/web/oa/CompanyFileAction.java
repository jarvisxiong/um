...
/**
 * 
 * 类名 CompanyFileAction 创建者 lijin 创建日期 2010-4-12 描述 管理体系Action类
 */
@Namespace("/oa")
@Results({ @Result(name = CrudActionSupport.RELOAD, location = "company-file.action", type = "redirect") })
public class CompanyFileAction extends CrudActionSupport<OaMeeting> {
	...
	public String loadFolderTree() throws Exception {
		...
		if (SpringSecurityUtils.hasRole("A_COMPANYFILE_ADMIN")) {
			folderJSON = oaFilingFolderManager.buildFolderTreeJSON(folderId,
					DictContants.COMPANY_FILING_MODULE_CD_FILE, null);
		} else {
			folderJSON = oaFilingFolderManager.buildFolderTreeJSON(folderId,
					DictContants.COMPANY_FILING_MODULE_CD_FILE, SpringSecurityUtils.getCurrentUiid());
		}
	...
	public String saveFolder() throws Exception {
		...
		folderEntity.setBizModuleCd(DictContants.COMPANY_FILING_MODULE_CD_FILE);
