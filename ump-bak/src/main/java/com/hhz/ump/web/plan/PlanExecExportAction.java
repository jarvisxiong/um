...
public class PlanExecExportAction extends ActionSupport{
	...
	private static final long serialVersionUID = 1806310776793895049L;
	...
	public String exportPlanExecPlanDetail() throws UnsupportedEncodingException{
		...
		//明细状态
		Map<String, String> statusMap = detailManager.buildPlanDetailStatusMap();
		
		//计划主表
		PlanExecutionPlanMain main = planExecutionPlanMainManager.getExecutionPlanMainByProjCd(projectCd,planTypeCd);
		
		//所有节点
		List<PlanProjectNodeRel> planNodes = planProjectNodeRelManager.getValidProjNodes(projectCd, planTypeCd, null,null, resOrgName,null,false,null,null);

		//业态
		List<PlanExecutionPlan> plans = planExecutionPlanManager.getPlansPerProjCd(projectCd, planTypeCd, true);
		...
		Map<String, PlanExecPlanDetail> planDetailMap = new HashMap<String, PlanExecPlanDetail>();
		String key  = null;
		for (PlanExecPlanDetail d : detailManager.getDetailsByProjPlanNodes(nodeIds, "Export")) {
			//{relId_planCd: detail}
		...
		is = new PlanExecExportUtil().createExcelPlanDetail(fileName, planNodes, plans, planDetailMap, statusMap, nodeMap);
		downFileName = new String(fileName.getBytes("GBK"), "ISO8859-1");
