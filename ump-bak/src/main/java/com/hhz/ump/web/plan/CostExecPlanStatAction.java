 ...
 */
@Namespace("/plan")
@Results( { @Result(name = "matrix", location = "/WEB-INF/content/plan/cost-exec-plan-stat-matrix.ftl", type = "freemarker") })
public class CostExecPlanStatAction extends ActionSupport {
	...
	private CostExecutionPlanMain project;
	...
	private List<CostExecPlanDetail> detailNodes = new ArrayList<CostExecPlanDetail>();
	private List<CostProjectNodeRel> ctrlNodes = new ArrayList<CostProjectNodeRel>();
	...
	public String portal() throws Exception {
		...
		if (StringUtils.isNotBlank(centerCd)) {
			List<Object> list = costExecutionPlanMainManager.getProjectListByResOrgCd(centerCd,planTypeCd);
			WsPlasOrg org = null;
...
//					}
		...
		// 项目公司
			...
			List<CostExecutionPlanMain> list = costExecutionPlanMainManager.getProjectListByProjectCd(projectCd);
			CostExecutionPlanMain obj = null;
			for (int i = 0; i < list.size(); i++) {
				obj = list.get(i);
		...
		// 管理员/所有用户
			...
			projects = costExecutionPlanMainManager.fetchAllProjects();
			if (StringUtils.isBlank(projectCd)) {
				projectCd = ZZ_PROJECT_CD;
			}
		}
...
//		infoInputor = permissions[2];
		...
		operations = planExecutionPlanManager.getPlansPerProjCd(projectCd, DictContants.PLAN_TYPE_EXC, true);//默认只有执行计划才看进度.
	...
	public String switchOperation() {
		...
		// 控制节点清单
		project = costExecutionPlanMainManager.getPlanMainByProjCd(projectCd);
		PlanExecutionPlan operationPlan = planExecutionPlanManager.getEntity(operationId);
		detailNodes = costExecPlanDetailManager.getSortedStatPlanNodeDetail(project.getProjectCd(),planTypeCd,operationPlan.getExecutionPlanCd());
		...
		CostExecPlanDetail d =null;
		for(int i=0;i< detailNodes.size();i++){
			d = detailNodes.get(i);
			String detailId = d.getCostExecPlanDetailId();
			String nodeId = d.getCostProjectNodeRel().getCostProjectNodeRelId();
	...
	public CostExecutionPlanMain getProject() {
		return project;
	}

	public void setProject(CostExecutionPlanMain project) {
	...
	public List<CostExecPlanDetail> getDetailNodes() {
		return detailNodes;
	}

	public void setDetailNodes(List<CostExecPlanDetail> detailNodes) {
	...
	public List<CostProjectNodeRel> getCtrlNodes() {
		return ctrlNodes;
	}

	public void setCtrlNodes(List<CostProjectNodeRel> ctrlNodes) {
