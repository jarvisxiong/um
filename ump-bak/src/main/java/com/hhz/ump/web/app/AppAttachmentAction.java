 ...
 */
@Namespace("/app")
@Results({ @Result(name = CrudActionSupport.RELOAD, location = "app-attachment!list.action", type = "redirect", params = {
...
public class AppAttachmentAction extends CrudActionSupport<AppAttachFile> {
	...
	public String delete() throws Exception {
		try {
			...
			appAttachFileManager.deleteAppAttachFile(appAttachFile, bizEntityName, bizEntityId, false);
	...
	private List<AppAttachFile> uploadFiles() throws Exception {
		...
		if (upload != null) { // upload multiple files
			...
			for (int i = 0; i < upload.length; i++) {
				if (upload[i] != null) {
					...
					if (!isAllow) {
						Struts2Utils.renderHtml("cantUploadType-"
								+ StringUtils.substringAfterLast(uploadFileName[i], "."));
						return null;
					} else {
		...
		// logger.info(uiid + "结束上传文件" + beginDate.toGMTString() + "一共耗时：" + (endDate.getTime() - endDate.getTime())/
		// 1000 + "s");
		return lstAdd;
	}
	 ...
	 * 增加下载次数
	 */
	private void addDownloadTimes(AppAttachFile attachFile, File file) {
		AppAttachFile attachFileTmp = attachFile;
		if (attachFileTmp == null) {
			attachFileTmp = appAttachFileManager.getAttachFile(file.getName());
			if (attachFileTmp == null) {
				attachFileTmp = new AppAttachFile();
				attachFileTmp.setFileName(file.getName());
				attachFileTmp.setBizModuleCd(bizModuleCd);
				attachFileTmp.setBizEntityId(RandomUtils.generateString(16));
				attachFileTmp.setRealFileName(getRealFileName());
				attachFileTmp.setFilePath(file.getPath());
				attachFileTmp.setFileTypeName(getContentType());
				attachFileTmp.setFileSize(new BigDecimal(file.length()));
				attachFileTmp.setBizFieldName(bizFieldName);
				attachFileTmp.setStatusCd(AppAttachFileManager.STATUS_NORMAL);// 1-正常，0-删除
			}
		}
		attachFileTmp.setDownloadTimes(attachFileTmp.getDownloadTimes() == null ? 1 : attachFileTmp.getDownloadTimes() + 1);
		appAttachFileManager.saveAppAttachFile(attachFileTmp);
		Struts2Utils.getRequest().setAttribute("error_attachFileId", attachFileTmp.getAppAttachFileId());
	}
	...
	public String download() throws Exception {
		try {
			...
			if (!file.exists()) {
				...
				if (!file.exists()) {
					...
					if (StringUtils.isNotEmpty(getId())) {
						attachFile = appAttachFileManager.getEntity(getId());
					...
					if (!file.exists()) {
						// 如果找不到，根据文件名寻找文件
						attachFile = appAttachFileManager.getAttachFile(fileName);
						if (attachFile != null) {
							file = new File(attachFile.getFilePath() + "\\" + fileName);
							if (!file.exists()) {
			...
			addDownloadTimes(attachFile, file);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return SUCCESS;
	...
	public String list() throws Exception {
		...
		if (StringUtils.isNotEmpty(bizFieldName) && !"null".equalsIgnoreCase(bizFieldName)
	...
	public String resShow() throws Exception {
		...
		List<PropertyFilter> filters = new ArrayList<PropertyFilter>();
		filters.add(new PropertyFilter("EQS_bizEntityId", bizEntityId));
		String bizFieldName = Struts2Utils.getParameter("bizFieldName");
		if(StringUtils.isNotBlank(bizFieldName)){
			filters.add(new PropertyFilter("EQS_bizFieldName", bizFieldName));
		}
		attachFiles = appAttachFileManager.find(filters);
		...
		for (AppAttachFile file : attachFiles) {
			if (DictContants.BIZ_MODULE_CD_RESCONTRACT.equalsIgnoreCase(file.getBizModuleCd())) {
				...
				jcwFiles.add(file);
			} else if (DictContants.BIZ_MODULE_CD_PROJECTMANAGER.equalsIgnoreCase(file.getBizModuleCd())) {
				otherFiles.add(file);
			} else {
	...
	public void updateAttach() throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		appAttachFileManager.updateWork();
	}
 */
}
