...
/**
 * 
 * 类名 CostExecPlanDetailAction 创建者 李劲 创建日期 2010-7-2 描述
 * 执行计划详情Action,提供对计划详情的操作,减轻CostExecutionPlanAction负担
 */
@Namespace("/plan")
@Results({ @Result(name = ActionSupport.SUCCESS, location = "cost-exec-plan-detail!exportExcel.action", type = "stream", params = {
		"contentType", "application/vnd.ms-excel", "contentDisposition", "attachment;filename=${excelFileName}.xls",
		"inputName", "excelFile" }) })
public class CostExecPlanDetailAction extends ActionSupport {
	...
	public String confirmPlanInfo() throws Exception {
		if (SpringSecurityUtils.hasRole(GlobalConstants.A_COSTP_SUP_ADMIN)) {
			if (costExecPlanDetailManager.confirmPlanDetailInfo(planDetailId)) {
	...
	public String bachConfirmPlanInfo() throws Exception {
		if (SpringSecurityUtils.hasRole(GlobalConstants.A_COSTP_SUP_ADMIN)) {
			if (costExecPlanDetailManager.batchConfirmPlanDetailInfo(projectCd, planTypeCd)) {
				Struts2Utils.renderText("succ");
			}
		}
		return null;
