...
function refreshFileList(folderId) {
	...
	$.post(_PATH + "/oa/company-standard!fileList.action?folderId=" + folderId, function(result) {
		if (result) {
...
function addNewFolder() {
	...
	$.get(_PATH + "/oa/company-standard!inputFolder.action", function(result) {
		if (result) {
			$("#rightContainer").html(result);			
		}
	});
...
function editFolder() {
	...
	$("#rightContainer").html("<div><div class='loading'></div></div>");
	$.get(_PATH + "/oa/company-standard!inputFolder.action?folderId=" + editedId, function(result) {
		if (result) {
			$("#rightContainer").html(result);			
		}
	});
...
function getParent() {
	...
	ymPrompt.confirmInfo({
		...
		afterShow:function(){
		$.get(_PATH + "/oa/company-standard!loadFolderTree.action?folderId=" + folderId, function(result) {
...
function saveFolder() {
	...
	//序号不能为空，否则构造树时出现异常!
	var seq = $('#sequenceNo').val();
	if($.trim(seq) == ''){
		$('#sequenceNo').val('0');
	}
	$("#inputFolderForm").ajaxSubmit(function(result) {
		alert('保存成功!');
		$('#inputFolderForm').hide();
		refreshFolderTree();
	});
}
// 清除上级目录输入框
...
function delFolder() {
	if (confirm("确认删除当前目录吗？") == true) {
		$("#inputFolderForm").attr("action", "company-standard!delFolder.action").submit();
	}
}
// 返回主页面
function back() {
	$("#inputFolderForm").attr("action", "company-standard.action").submit();
}
// 更新目录树
function refreshFolderTree(type) {
	...
	if($('#load_auto_flag').attr('isfirst') !='1'){
		$('#load_auto_flag').attr('isfirst','1');//设置第一次加载完成
	}else{
		//若不勾选,不刷新
	...
	$("#foldersTree").html('<div class="loading">加载目录...</div>');
	
	$.post(_PATH + "/oa/company-standard!loadFolderTree.action", {treeType : treeType}, function(result) {
		$("#foldersTree").html("");
		if ($.trim(result) != "") {
...
function addFile(folderId) {
	...
	ymPrompt.confirmInfo({
		...
		handler:saveFile,
		afterShow:function(){
			$.get(_PATH + "/oa/company-standard!inputFile.action?folderId=" + folderId, function(result) {
...
function editFile(fileId) {
	...
	ymPrompt.confirmInfo({
		...
		handler:saveFile,
		afterShow:function(){
			$.get(_PATH + "/oa/company-standard!inputFile.action?folderId=" + folderId + "&fileId=" + fileId, function(result) {
				if (result) {
...
function delFile(fileId, folderId) {
	if (confirm("确认删除该文件吗?")) {
		$.post(_PATH + "/oa/company-standard!deleteFile.action?fileId=" + fileId, function(result) {
			if (result == "done") {
				refreshFileList(folderId);
			}
		});
...
function uploadAttach(fileId) {
	...
	$("#inputUploadAttForm").ajaxSubmit(function(result) {
		if (result) {
			var url = _PATH + "/oa/company-standard!fetchAttachList.action?bizEntityId=" + fileId;
...
function deleteAttachment(attachId, bizEntityId) {
	...
	$.post(url,
		...
		function(result) {
			if (result) {
				var url = _PATH + "/oa/company-standard!fetchAttachList.action?bizEntityId=" + bizEntityId;
				$.get(url, function(result) {
					if (result) {
						$("#inputAttachList").html(result);
					}
...
function jumpTo(folderId, pageNo) {
	...
	if (p && t) {
		...
		$.get(_PATH + "/oa/company-standard!fileList.action?folderId=" + folderId + "&filePage.pageNo=" + pageNo, function(result) {
			if (result) {
				$("#rightContainer").html(result);
			}
		});
...
function openAtta(url,fileName){
	...
	if(suffix == 'pdf'){
		window.parent.TabUtils.newTab("pdf-companyStandard-atta","查看附件",url,true);
	}else{
		window.open(url);
	}
}
