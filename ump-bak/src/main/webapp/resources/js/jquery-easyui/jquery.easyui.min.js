...
},beforePageText:"当前第",afterPageText:"/{pages}页",displayMsg:"共 {total} 页"};
})(jQuery);
(function($){
function _7c(_7d){
var _7e=$(_7d);
...
var _43e="";
if(opts!=null && opts.multiple=="true"){
_43e=$(ed.target).next().find("input").val(); 
}else{
_43e=ed.actions.getValue(ed.target);
}
//关联的字段
if(opts!=null && opts.relationField){
	...
	if(fields.length>1){
		//IDS
	row[fields[0]]=ed.actions.getValues(ed.target);
	//值
	row[fields[1]]=_43e;
	}
}
...
var _485={datetext:{init:function(_486,_487){
...
},getValues:function(_4bf){
return $(_4bf).combobox("getValues");
...
if(row.showTreeIcon!=undefined && row.showTreeIcon==false){
	//如果不显示文件图标	
 if(row.state=="closed"){
cc.push("<span class=\"tree-hit tree-collapsed\"></span>");
cc.push("<span class=\"\"></span>");
}else{
if(row.children&&row.children.length){
...
if(opts.multiple){	
//begin qlb
$(_67d).attr("isMultiple",opts.multiple);
//是否使用自己的多选值设置
	if(opts.isUseMyselfSet){
		...
		var _text="";
		$(_67d).parent().find(".combo-value").remove();
		var _itmDivs=$(item).parent();
		$.each(_itmDivs.find("div.combobox-item-selected"),function(i,itemDiv){
			
		var _0927d=$("<input type=\"hidden\" class=\"combo-value\" >").appendTo($(_67d).parent());
		...
		$(_67d).parent().find("span input").val(_text);
	}else{
		//系统原始代码
	     if(item.hasClass("combobox-item-selected")){
...
//begin qlb
if(opts.isCallParam && !opts.isCallParam.values){
	opts.isCallParam.values=item.attr("value");
	opts.isCallParam.texts=item.attr("text");
}
if(opts.relationField){
$(_67d).attr("relationField",opts.relationField);
}

if(opts.callSelected){

 window.setTimeout(opts.callSelected,10);
}
