...
function saveNote(){
	...
	if(!IS_NOTE_DIRTY){
		if('' == content){
			$('#'+id).attr({'className':'note note_empty'});
		}
		closeNote();
		return;
	}
	$.post(_ctx+'/dly/dly-note!save.action',{noteContent:content,tipStartDate:remindDate,id:id},function(result){
		...
		$this.attr({'className':'note'});
		delete noteTipObj[currentNoteTipDate];//删除当前提醒缓存
		if('' == tipDate){
			$this.attr({'className':'note note_noremind'});
		}else{
			//添加到提醒缓存
		...
		if('' == obj[2]){
			$this.attr({'className':'note note_empty'});
		}
		closeNote();
	});
}
...
function delNote(){
	...
	$.post(_ctx+'/dly/dly-note!save.action',{noteContent:'',tipStartDate:'',id:id},function(result){
		IS_NOTE_DIRTY = false;
		var obj = eval(result);
		$('#'+id).attr({'title':'','className':'note note_empty'});
		$('#'+id+' div').text('');
		closeNote();
		delete noteTipObj[currentNoteTipDate];//删除当前提醒缓存
	});
...
function showSecondPage(dom){
	$('#note_page_1').hide();
	$('#note_page_2').show();
	$('#note_page_btn_1').removeClass("page_active");
	$('#note_page_btn_2').addClass("page_active");
	//$(dom).hide().next().show();
}
/**
 * 显示第一页
...
function showFirstPage(dom){
	$('#note_page_2').hide();
	$('#note_page_1').show();
	$('#note_page_btn_2').removeClass("page_active");
	$('#note_page_btn_1').addClass("page_active");
	//$(dom).hide().prev().show();
}
