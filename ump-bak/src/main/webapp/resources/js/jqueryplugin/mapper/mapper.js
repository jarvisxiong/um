...
/**
 * VML是The Vector Markup Language(矢量可标记语言)的缩写。VML其实是Word和HTML结合的产物
 */
/**
 * gmipam_0_canvas: 鼠标滑过显示的画布
 * gmipam_0_canvas_select：点选指定区域绘画的画布
 * gmipam_0_image: 初始化显示的画布
 * 创建新画布的方法：
 * 1 创建画布：var  XXX_canvas = document.createElement('canvas');
 * 2 定义画布属性：例如
 * 		//image  指定是：gmipam_0_image
 * 		XXX_canvas.id = 'XXX_canvas';
 * 		XXX_canvas.style.height = image.height+'px';
		XXX_canvas.style.width = image.width+'px';
		XXX_canvas.height = image.height;
		XXX_canvas.width = image.width;
		XXX_canvas.left = 0; XXX_canvas.top = 0;
		XXX_canvas.style.position = 'absolute';
		XXX_canvas.style.left = 0+'px';
		XXX_canvas.style.top = 0+'px';
		XXX_canvas.fading = 0;
	3 插入新创建画布：
		object.insertBefore(XXX_canvas,image);
 */
var isVM = isIE&&document.namespaces?1:0; 
var iniHtml='';//初始化平面图使用此字符串保存ie处理过的innerHTML
var isJG = 0;
//画布上下文
...
function splitt(ss,flag1,flag2){
	var v = ss.split(flag1);
	var v2 = new Array();
	for( var i=0;i<v.length;i++){
		var v1 = v[i].split(flag2);
...
function initAreaOver(obj,id,bd,co,op,nb,f,z) {
	if(isVM) {
		setAreaOver(obj,id,bd,co,op,nb,f,z,'1');
	}else{
 ...
 * f	nofade
 * z	序号
 * ini	标示是否使用初始化暂存字符串iniHtml
 */
function setAreaOver(obj,id,bd,co,op,nb,f,z,ini) {
	var a, i, j, d, c, o, b, n, l, r, v, u, x, y, p, k = 0, t = '', r = obj.getAttribute('rel'), 
	...
	if(isVM) {//ie
		for(a=0;a<d.length;a++) {
			...
			if(obj) { 
				p = '', l = obj.className, u = l.split(" "), v=splitt(obj.coords,',',' ');//v = obj.coords.split(",");
				if(k==0) {setAttr();} 
		...
		if(ini){
			...
			canvas.innerHTML.replace(/<\/v:roundrect>/, " ");
			canvas.innerHTML += t+"</v:roundrect>";
		}
		...
		for(a=0;a<d.length;a++) {
			obj = document.getElementById(d[a]); 
				if(obj) {
				l = obj.className, u = l.split(" "), v=splitt(obj.coords,',',' ');//v = obj.coords.split(",");
...
function addMapper() {
	...
	var image, object, bgrnd, canvas,canvas_select, blind, context, mapid, mname, ele, atr; 
	...
	for(i=0;i<themaps.length;i++) {
		...
		if(isCV) {//firefox
			...
			canvas_select = document.createElement('canvas');
		}else if(isVM) {
			...
			canvas_select = document.createElement(['<var style="zoom:1;overflow:hidden;display:block;width:'+image.width+'px;height:'+image.height+'px;padding:0;">'].join(''));
		}else {
			canvas = document.createElement('div');
			canvas_select = document.createElement('div');
		}
		canvas.id = image.id+'_canvas';
		canvas_select.id = image.id+'_canvas_select';
		//画布样式
		classes = image.className.split(' '); 
		r = getClassValue(classes,"iradius");//半径, 范围, 辐射光线, 有效航程, 范围, 界限
		o = getClassValue(classes,"iopacity");//透明度
		b = getClassRGBColor(classes,"iborder",'006fb6');
		c = getClassRGBColor(classes,"icolor",'006fb6');
		...
		canvas_select.style.height = image.height+'px';
		canvas_select.style.width = image.width+'px';
		canvas_select.height = image.height;
		canvas_select.width = image.width;
		canvas_select.left = 0; canvas_select.top = 0;
		canvas_select.style.position = 'absolute';
		canvas_select.style.left = 0+'px';
		canvas_select.style.top = 0+'px';
		canvas_select.fading = 0;
		...
		object.insertBefore(canvas_select,image);
		...
		blind.innerHTML = " ";
		//在背景图上插入画布：隐藏在区域边界  在指定的节点前插入一个子节点。
		...
		if(isCV && r>0) {//firefox
			...
			bgrnd.innerHTML = '<v:roundrect arcsize="'+r+'" strokeweight="0" filled="t" stroked="f" fillcolor="#ffffff" style="zoom:1;margin:0;padding:0;display:block;position:absolute;left:0px;top:0px;width:'+bgrnd.width+'px;height:'+bgrnd.height
				+'px;"><v:fill src="'+image.src
...
//attachEvent(IE)	使用这个方法，可以一个事件，绑定多个处理过程。 事件是以"on"开头的
//addEventListener(firefox)
/**
 * 在多个浏览器下初始化addMapper方法
 */
