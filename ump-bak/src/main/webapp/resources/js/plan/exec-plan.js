...
var now_view_style = 2;			//现在的显示模式，默认为默认状态
var now_res_org_names = "";		//现在搜索的主责方
var if_bis = false;				//是否是商业的计划
...
function switchProject(srcEle, curProjCd) {
	...
	isLoading = true;
	var url  = _PATH + "/plan/exec-plan!planMatrix.action";
	if("5"==getPlanTypeCd()){
		//如果是项目开发计划，默认是3级节点
		now_point_level = 3;
	}
...
function svNodeCompDate(nodeId) {
	...
	if (newVal != oldVal) {
		$.post(_PATH + "/plan/exec-plan!saveCompDate.action",
				{
					projNodeId 			: nodeId,
...
function configPlan() {
	...
	var url  = _PATH + "/plan/exec-plan!configPlan.action";
...
function addPlanModule(projPlanId) {
	...
	var url  = _PATH + "/plan/exec-plan!saveProjPlan.action";
	var data = {
			projectCd	:	curProjCd,
			planTypeCd  :   planTypeCd,
			projPlanId 	: 	"",
...
function savePlanName(planId) {
	...
	var sequenceNo = $("#" + planId + "sequenceNo").val();
...
// 更改状态（激活/禁用）
function changePlanState(planId, curState,if_bis) {
	if (!curState) {
		return;
	}
	if("true"==if_bis){
		alert("此业态已被商业应用，不可修改。如要修改请联系营运管理中心PD开发组");
		return;
	}
	
	var newState = curState == "1" ? "0" : "1"; 
	var data = {projPlanId : planId,projPlanState : newState };
	$.post(_PATH + "/plan/exec-plan!changePlanState.action", data, function(result) {
		if (result == "succ") {
			configPlan();
		}
	});
...
function mngProjReminder() {
	...
	var planTypeCd = getPlanTypeCd();
	var murl  = _PATH + "/plan/exec-plan!fetchProjReminders.action";
...
function saveProjReminders(tp){
	if (tp=="ok") {
		...
		var url = _PATH + "/plan/exec-plan!saveProjReminders.action";
...
function mngNodeReminders(nodeRelId) {
	...
	var myUrl  = _PATH + "/plan/exec-plan!fetchNodeReminders.action";
...
function saveNodeReminders(tp){
	if (tp=="ok") {
		...
		$.post(_PATH + "/plan/exec-plan!saveNodeReminders.action", 
				{
...
//展示节点详情
function showPlanDetail(srcEle, nodeId, layoutId, detailId) {
...
function savePlanDetail(tp) {
	...
	$("#planDetailForm").ajaxSubmit(function(result) {
		$.post(_PATH + "/plan/exec-plan!fetchPlanDetailInfo.action", 
		{
		...
		function (result) {
			if ("failure"==result) {
				...
				if("10"==getPlanTypeCd()
						...
						||"24"==getPlanTypeCd()){
					//如果是商业的计划，刷新的时候的设置
					$("td[id='" + key + "_data1_td']").html("");
					$("td[id='" + key + "_data2_td']").html("");
					$("td[id='" + key + "_data3_td']").html("");
				...
				if("10"==getPlanTypeCd()
						...
						||"24"==getPlanTypeCd()){
					//如果是商业的计划，刷新的时候的设置
					$("td[id='" + key + "_data3_td']").html(result.split("*")[0]);
					$("td[id='" + key + "_data1_td']").html(result.split("*")[2]);
					$("td[id='" + key + "_data2_td']").html(result.split("*")[3]);
				...
				try{
					var attachFlg = result.split("*")[3];
					if(1==attachFlg){
						$("td[id='" + key + "_data3_td']").html($("td[id='" + key + "_data3_td']").html()+"<img src='../resources/images/common/atta_y.gif'/>");
					}
				}catch(e){}
			}
		});
...
function checkFileType(fileName){
	...
	if(name=="doc"||name=="docx"||name=="xls"||name=="xlsx"||name=="pdf"||name=="ppt"||name=="pptx"||name=="jpg"){
		return true;
	}else{
		return false;
	}
...
function logOutputUpdateHistory(planDetailId, updateType, fileId) {
	/*
	$.post(_PATH + "/plan/exec-plan!logOutputUpdateHistory.action",
			{
...
function refreshOutputList(bizEntityId) {
	...
	$.post(_PATH + "/plan/exec-plan!fetchOutputList.action",
			{
...
function deletePlanDetail(srcEle, planDetailId){
 if(planDetailId){
	 if(confirm("确定要清空这条记录吗？")){
		$.post(_PATH + "/plan/exec-plan!deletePlanExecPlanDetail.action",
		{
			 planDetailId 	: planDetailId
		},	
		function (result) {
...
function completePlan(srcEle, planDetailId) {
	...
	$.post(_PATH + "/plan/exec-plan!checkIfCanComplete.action",
	{
		planDetailId 	: planDetailId
	},
	function (result) {
...
function changePlanDetailStatus(srcEle, planDetailId, status) {
	...
	srcEle.attr("disabled", "disabled");
	$.post(_PATH + "/plan/exec-plan!changePlanDetailStatus.action",
	{
		planDetailId 	 : planDetailId,
...
function confirmInfo(planDetailId,confirmFlg) {
	if (planDetailId) {
		if(null==confirmFlg || ""==confirmFlg){
			confirmFlg = "1";
		}
		$.post(_PATH + "/plan/exec-plan!confirmPlanInfo.action", { planDetailId : planDetailId,confirmFlg:confirmFlg },
			function (result) {
				if (result == "succ") {
					alert("信息审核确认成功");
					if("1"==confirmFlg){
						...
						$("#infoConfirmTd").html("<span style='color:red'>未确认</span>");
					}
				}
		});
	}
...
function batchConfirm() {
	...
	if (confirm("确定所有的"+desc+"都是正确吗？")) {
		var projCd = getCurProjectCd();
		var planTypeCd = getPlanTypeCd();
		var url  = _PATH + "/plan/exec-plan!bachConfirmPlanInfo.action";
...
function configNodes() {
	...
	var planTypeCd= getPlanTypeCd();
	$("#planContainerDiv").html('<div class="waiting" style="width:200px;height:100px;"></div>');
	var url  = _PATH + "/plan/exec-plan-node!list.action";
	var data = {projectCd:projCd,planTypeCd:planTypeCd};
	$.post(url, data,function(result) {
		$("#planContainerDiv").html(result).hide().fadeIn();
...
function exportExcel() {
	var url = _PATH + "/plan/exec-plan!exportExcel.action";
	this.location.href=url;
}
function fetchCenterDetails(){
	var url =_PATH + "/plan/exec-plan!getCenterPlanRel.action?centerCd="+"11";
	this.location.href=url;
}
function returnMain(){
	var url =_PATH +"/plan/exec-plan!portal.action";
	this.location.href=url;
}
function gotoExecStat(centerCd,projectCd){
	var url =_PATH +"/plan/exec-plan-stat!portal.action?centerCd="+centerCd+"&projectCd="+projectCd;
	showExecStat(url);
}
function getFullSrc(projectCd,centerCd){
	if(projectCd!=""){
		var url = _PATH + "/plan/exec-plan!getCenterPlanRel.action?projectCd=" + projectCd;
		window.open(url);
	}
	if(centerCd!=""){
		var url = _PATH + "/plan/exec-plan!getCenterPlanRel.action?centerCd="+centerCd;
		window.open(url);
	}
}
...
function projectExecStat(){
	...
	var planTypeCd = getPlanTypeCd();
	var url = _PATH +"/plan/exec-plan-stat!portal.action?projectCd="+projectCd +"&planTypeCd="+planTypeCd;
	showExecStat(url);
}
...
//	alert(getCurProjectCd()+",planTypeCd:"+getPlanTypeCd());
	if(getPlanTypeCd() == "1"){
		//alert(getCurProjectCd()+",planTypeCd:"+getPlanTypeCd());
		window.open(_ctx + "/plan/exec-plan!exportPlanExecPlanDetail.action?projectCd="+getCurProjectCd());
	}else if(getPlanTypeCd() == "16"){
		window.open(_ctx + "/plan/exec-plan!exportPlanExecBiz16.action?projectCd="+getCurProjectCd()+"&planTypeCd="+getPlanTypeCd());
	}
	else{
		//alert(getCurProjectCd()+",planTypeCd:"+getPlanTypeCd());
		window.open(_ctx + "/plan/exec-plan!exportPlanExecBiz4.action?projectCd="+getCurProjectCd()+"&planTypeCd="+getPlanTypeCd());
	}
}
...
function onClickCheckTreeType(inputVal){
	...
	if("5"==inputVal){
		...
		$("#btnPointLevel5").addClass("button_blue");
	}else if("1"==inputVal || "2"==inputVal ||"3"==inputVal){
		setPlanTypeCd(1);
		now_point_level = inputVal;
		for(var i=1;i<4;i++){
			$("#btnPointLevel"+i).removeClass();
			if(i==now_point_level){
				$("#btnPointLevel"+i).addClass("button_blue");
			}else{
				$("#btnPointLevel"+i).addClass("btn_white");
			}
		}
	...
	}else if("10"==inputVal || "11"==inputVal || "14"==inputVal || "15"==inputVal ||"16"==inputVal ||"24"==inputVal){
		setPlanTypeCd(inputVal);
		now_point_level = inputVal;
		for(var i=10;i<17;i++){
			if("10"==i || "11"==i || "14"==i || "15"==i ||"16"==i ||"24"==i){
				try{
					$("#btnPointLevel"+i).removeClass();
					if(i==now_point_level){
...
function onClickViewStyle(viewStyle){
	if(viewStyle!=now_view_style){
		...
		for(var i=1;i<3;i++){
			...
			if(i==now_view_style){
				$("#btnViewStyle"+i).addClass("button_blue");
			}else{
...
function saveMessage(detailId){
	...
	if($.trim(newMessage).length>0){
		...
		isLoading = true;
		$.post(_ctx+"/plan/exec-plan!saveMessage.action",
				{
					planDetailId:detailId,
...
//保存留言
function saveMessage1(detailId){
	var newMessage = $("#" + detailId + "commentArea").val(); 
	if($.trim(newMessage).length>0){
		if(isLoading){
			return;
		}
...
//留言查看
function doMessageView(){
	$.post(_ctx+"/plan/exec-plan!getAllSuspendExecPlan.action",
			{
				flags:1
			},
			function(result) {
				 $("#oaMeetingContainer").html(result);
			});
}
function doJinduViews(){
	$.post(_ctx+"/plan/exec-plan!getAllSuspendExecPlan.action", function(result) {
		 $("#oaMeetingContainer").html(result);
	});
...
function centerClick(search_year,search_planMonth,search_centerCd){
	if(!IF_IN_LOADING){
		...
		if (now_year==0){
			now_year = $("#select_now_year").val();
		}
		var curProjCd=$("#typecd_select").val();
		curOpenedNodeCompDateId = null;
//		alert("now_year:"+now_year+",now_month:"+now_month+",curProjCd:"+curProjCd);
		$("#planContainerDiv").html('<div class="waiting" style="width:200px;height:100px;"></div>');
		var data = {
				projectCd : curProjCd,
				isChange : 'true',
		...
		$.post(_ctx+"/plan/exec-plan-extend!dataMissingAnalysis.action", data, function(result) {
			$("#planContainerDiv").html(result).hide().fadeIn();
		});
	}
}
//签约信息的新增保存
function doSavePlus(execPlanDetailPlusId,execPlanDetailId,status){
	var shopName,scheduleEndDate,realEndDate;
	if(null!=execPlanDetailPlusId){
		//修改
		shopName = $("#shopName_"+execPlanDetailPlusId).val();
		scheduleEndDate = $("#scheduleEndDate_"+execPlanDetailPlusId).val();
		realEndDate = $("#realEndDate_"+execPlanDetailPlusId).val();
	}else{
		//新增
		execPlanDetailPlusId = "";
		shopName = $("#shopName_add").val();
		scheduleEndDate = $("#scheduleEndDate_add").val();
		realEndDate = $("#realEndDate_add").val();
	}
	$.post(_ctx+"/plan/exec-plan!doSavePlus.action",
			{
		execPlanDetailPlusId:execPlanDetailPlusId,
		shopName:shopName,
		scheduleEndDate:scheduleEndDate,
		realEndDate:realEndDate,
		status:status,
		planDetailId:execPlanDetailId
			},
			function(result) {
				$("#planDetailDiv").html(result);
			});
 ...
 */
function moveRight(){
	var width=$("#mainPlanMatrixDiv").css("margin-left").replace("px","");
	var nowWidth =eval(width)-80;
	//var totalWidth =$("#topRightTitle").css("width").replace("px","");
	//totalWidth=totalWidth+333;
	//alert($(document).width()+","+totalWidth);
	if((0-nowWidth)<($(document).width()-330)){
		$("#mainPlanMatrixDiv").css("margin-left",nowWidth+"px");
	}
}
function moveleft(){
	var width=$("#mainPlanMatrixDiv").css("margin-left").replace("px","");
...
function autoHeightExec(){
	...
	try{
		...
		if(null!=menuId && undefined !=menuId){
			...
			if(endHeight>604){
				$(window.top.document).find("#bodyLoad").height(660);
				...
				$(window.top.document).find('#' + menuId+'_frame').height(622);
				$(window.top.document).find('#' + menuId+'_frame').css("overflow","scroll");
			}
		}
	}catch(e){}
}
