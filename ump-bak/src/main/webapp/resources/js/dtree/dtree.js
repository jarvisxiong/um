...
Node.prototype.setNodeIcon = function(iconFullPath){
	this.icon = iconFullPath;
};
 ...
 * 定义树型结构
 */
function dTree(objName,iconPath,_ctx) {
	this.config = {
		ctx				: _ctx,
		imgRootPath		: null,
		target			: null,
	...
	if(!iconPath) this.iconPath="img/";//set the default value, be compatible with dtree
	
	this.config.imgRootPath = iconPath;
	//alert(this.config.imgRootPath);
	
	//默认图片的路径，注意要指对
	this.icon = {
			root		: iconPath+'empty.gif',//base.gif
			folder		: iconPath+'triangle_right.jpg',//folder.png
			folderOpen	: iconPath+'triangle_down.jpg',//folderopen.png
			node		: this.config.ctx+'/images/webim/none_offline.jpg',//leaf node
			...
			//男女同胞的图片
			male		: this.config.ctx+'/images/webim/male_offline.jpg', 
			female		: this.config.ctx+'/images/webim/female_offline.jpg', 
...
dTree.prototype.add = function(id, pid, name, url, title, target, icon, iconOpen, open, extParam) {
	var cn = new Node(id, pid, name, url, title, target, icon, iconOpen, open, extParam);
	cn.setNodeType(this.delegateGetNodeType(cn));
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
		...
		//add space for leaf object
		var leafPreHtml = this.delegateGetLeafPreHtml(node);
		str += leafPreHtml;
		var nodeIcon = (node.icon == this.icon.root)?node.icon:this.delegateGetLeafImg(node);
		str += '<img id="i' + this.obj + nodeId + '" nodetype="'+node.nodeType+'" nodeid="'+node.id+'" nodeextparam="'+node.extParam+'" src="' + ((node._io)?node.iconOpen : nodeIcon) + '" alt="" />';
...
dTree.prototype.isIconFolder=function(imgPath){
	if( this.icon.folder == imgPath){
		//alert("this.icon.folder : " + this.icon.folder);
...
//要求人员图标前面增加一空格,以区分机构
dTree.prototype.delegateGetLeafPreHtml=function(cn){
	return "";
}; 
...
dTree.prototype.getCfgIconRootPath = function(){
	return this.config.imgRootPath;
};
//设置默认叶子图标
dTree.prototype.setCfgImgIcon=function(iconName,imgRootPath){
	if((typeof(imgRootPath) == "undefined") || imgRootPath == null){
	   this.icon.node = this.getCfgIconRootPath() + iconName;
	}else{
	   this.icon.node = imgRootPath + iconName;
	}
}; 

//获取节点类型,一般从extParam中获取节点类型
dTree.prototype.delegateGetNodeType = function(cn){
	return null;
};
