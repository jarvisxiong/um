	...
	$.extend({
		ouSelect : function(dom,opts){
			...
			function getUsersByOrg(node,tag){
				...
				//系统职位树
				$.post(option.ctx+'/com/select!getUsersbyOrgPos.action',param,function(result){
			...
			function initEvents(){
				...
				$('#btnPermCancel').click(function(){
					//add by huangbijin 2012-05-06 取消选择则清空
					$('input:checked','#popSelPos').each(function(){
						$(this).attr('checked','');
					});
					$('#deptDivPanel').show().siblings().hide();
				});
			...
			function userCallback(){
				...
				//点击确定回调函数 修改人 liuzhihui 2012-05-23
				if(option.callfun!=undefined){
					//执行回调函数
					window.setTimeout(option.callfun, 100);
				}
				$(jqmDlg).jqmHide();
			}
			//机构选择框确定事件
