...
loadCss(_ctx + '/resources/css/common/select.css');
loadCss(_ctx + '/resources/css/common/TreePanel.css');
...
loadJs(_ctx + '/resources/js/common/TreePanel.js');
    ...
    $.extend({
        ouSelect:function (dom, opts) {
            ...
            var defaultOpt = {
                ...
                sysPosNFiled:'sysPosName', //实体类属
                sysPosField:'sysPosCd', //实体属性
                sysOrgName:'orgName', //实体属性
            ...
            if (option.showCenterOrg || option.showProjectOrg || option.type == "org") {
                ...
                tPopBody.push('     <img style="vertical-align: middle;cursor:pointer;" src="' + _ctx + '/images/webim/search_zoom.jpg" onclick="$(this).prev().trigger(\'keyup\')" title="搜索下一个"/>');///PowerDesk
            ...
            function getPermCds() {
                var cds = new Array();
                $('#permDivPanel input:checked').each(function () {
                    cds.push($(this).val());
                    cds.push(',');
                });
                return cds.join('');
            ...
            function getUsersByOrg(node, tag) {
                ...
                var param = {
                    orgCds:orgCds.join(','),
                    showSysPos:(option.sysPosEId ? true : false),
                    permCds:getPermCds()
                };

                $.post(option.ctx + '/com/select!getUsersbyOrg.action', param, function (result) {
            ...
            function initEvents() {
                ...
                //按职级
                $('#btnFilterPerm').click(function () {
                    $("#permDivPanel").slideDown("fast").siblings().hide();
                });
                $('#btnPermOk').click(function () {
                    $('#deptDivPanel').show().siblings().hide();
                    //initOrgTree();
                });
                ...
                //按部门
                $('#btnFilterDept').click(function () {
                    $('#deptDivPanel').show().siblings().hide();
                });

                //按自定义组
                $('#btnFilterGroup').click(function () {
                    $("#groupDivPanel").show().siblings().hide();
                });

                //自定义组选择人员
                $('#groupDiv input').click(function () {
                    showMember('group', this, true);
                });
                $('#groupDiv span').click(function () {
                    showMember('group', this);
                });
                //按照职级选择人员
                $('#rankDiv input').click(function () {
                    showMember('rank', this, true);
                });
                $('#rankDiv span').click(function () {
                    showMember('rank', this);
                });
            }
            ...
            function userCallback() {
                ...
                for (var k in _userMap) {
                    ...
                    cds.push(obj["uiid"]);
                    postions.push(obj["sysPosCd"]);
                    postNames.push(obj["sysPosName"]);
                ...
                /*
                 ...
                 postNames.push(obj[option.sysPosName]);
                 orgNames.push(obj[option.sysOrgName]);
                 }*/
                ...
                if (names.length > 0) {
                    ...
                    postNameStr = postNames.join(';');
                    orgNameStr = orgNames.join(';');
                    if (option.muti) {
                        nameStr += ';';
                        cdStr += ';';
                        postionStr += ';';
                ...
                //alert("option.sysPosName:"+option.sysPosName);
                if (option.sysPosName) {
                    $('#' + option.sysPosName).val(postNameStr);
                }
                //alert("option.sysOrgName:"+option.sysOrgName);
                if (option.sysOrgName) {

                    $('#' + option.sysOrgName).val(orgNameStr);
                }
                //alert("option.cdField:"+option.cdField);
                if (option.cdField) {
                    $('#' + option.cdField).val(cdStr);

                }
                //alert("option.nameEId:"+option.nameEId);
                if (option.nameEId) {
                ...
                //alert("option.sysPosEId:"+option.sysPosEId);
                if (option.sysPosEId) {
                    $('#' + option.sysPosEId).val(postionStr);
                }
                //alert("$.isFunction(option.callback):"+$.isFunction(option.callback));
                if ($.isFunction(option.callback)) {
                    option.callback(_userMap, _this);
                }
                $(jqmDlg).jqmHide();
            }
            ...
            function addAndBindUser(objects, addFlag, checked) {
                ...
                function bind(obj, flg, cFlg) {
                    ...
                    if (userObj) {
                        objUser.attr('class', 'userSelected');
                    }
                    if (cFlg == '0') {
                        if (userObj) {
            ...
            $(_this).click(function () {
                var url = option.ctx + '/com/select.action?pageType='
                    ...
                    + '&showRank='
                    + option.showRank;
                if (option.defaultParentOrgCd != '') {
                    url = url + '&defaultParentOrgCd=' + $("#" + option.defaultParentOrgCd).val();
                }
                $(jqmDlg).jqm({
                    trigger:false,
                    ajax:url,
