...
K.options = {
	...
	items : [
		...
		'flash', 'media', 'insertfile', 'table', 'hr', 'mark', 'map', 'code', 'pagebreak', 'anchor', 'link', 'unlink', '|', 'about'
	],
...
function _copyAndDelete(range, isCopy, isDelete) {
					...
					if (incStart >= 0 && incEnd <= 0) {
						if (isCopy) {
							...
							if(node.nodeName.toLowerCase() == "table"){
								  $(node).attr("tbid",""+(new Date().getTime()));
								//选择中表格后将表格单独提取出来
								haveTable = true;
								//获取表格开始偏移量
								range.setStart(doc.body, testRange.startOffset);
								//获取表格结束偏移量
								range.setEnd(doc.body, testRange.endOffset);
								return;
						...
						if (isCopy) {
							...
							if(node.offsetParent.tagName.toLowerCase() == "table"){
								//获取正确偏移量
                                childFlag = node.offsetParent.cloneNode(true);
                                pNode = node.offsetParent;                            
                                  $(node.offsetParent).attr("tbid",""+(new Date().getTime()))
                                /*
                                 * 重新获取到表格位置
                                 * 如果其parentNode为ins则取父节点
                                 * */
                                if(pNode.parentNode.tagName.toLowerCase() == "ins"){
                                	testRange = new KRange(doc).selectNode(pNode.parentNode);
                                }else{
                                	testRange = new KRange(doc).selectNode(pNode);
                                }
                                
                                childFlag = node.cloneNode(true); 
                                //选择中表格后将表格单独提取出来
      							haveTable = true;
      							...
      							range.setEnd(doc.body, testRange.endOffset);
      							return;
                              }else if(node.offsetParent.tagName.toLowerCase() == "strong"){
                            	//获取正确偏移量
                                  childFlag = node.offsetParent.cloneNode(true);
                                  pNode = node.offsetParent;                            
                                    $(node.offsetParent).attr("tbid",""+(new Date().getTime()))
	...
	//alert(range.htmlText.toString());
    extractNodes(ancestor, frag);
	
	
	//extractTestNodes(ancestor, frag);
	
	//选中表格时单独提出表格
	if(haveTable){
		//重新初始化
		copyRange = range.cloneRange().down();
		start = -1, incStart = -1, incEnd = -1, end = -1,
		ancestor = range.commonAncestor(), frag = doc.createDocumentFragment();
		extractNodes(ancestor, frag);
	}
	if (isDelete) {
...
_extend(KRange, {
	...
	insertNode : function(node) {
		...
		if (sc.nodeType == 1) {
			...
			} else if (sc.nodeValue != null && so >= sc.nodeValue.length) {
				if (sc.nextSibling) {
...
_extend(KCmd, {
	...
	markcolor:function(a,id){//设置标注背景色
			
			return this.toggle('<span mkid="'+id+'" optype="bookmark" style="background-color:' + a
								+ ';"></span>', {
...
_extend(KEdit, KWidget, {
	init : function(options) {
		...
		self.iframe = K('<iframe class="ke-edit-iframe" hidefocus="true" id="keEditIframe" frameborder="0"' + iframeSrc + '></iframe>').css('width', '100%');
			...
			if (_IE) {
				K(doc).keydown(function(e) {
					...
					if(e.which == 13){
						...
						cmd.selection();
						var v = '<br />';
						var rng = cmd.range.get();
						if (rng.item) {
							rng.item(0).outerHTML = '<br />';
						} else {
							rng.pasteHTML(v);
						}
					}
