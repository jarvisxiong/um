...
var arrFontsize=[{n:'10px',wkn:'small',s:'10px',t:'特小'},{n:'12px',wkn:'medium',s:'12px',t:'小'},{n:'14px',wkn:'large',s:'14px',t:'中'},{n:'18px',wkn:'x-large',s:'18px',t:'大'},{n:'20px',wkn:'xx-large',s:'20px',t:'特大'}];
...
var arrTools={GStart:{},GEnd:{},Separator:{},Cut:{t:'剪切 (Ctrl+X)'},Copy:{t:'复制 (Ctrl+C)'},Paste:{t:'粘贴 (Ctrl+V)'},Pastetext:{t:'粘贴文本'},Blocktag:{t:'段落标签'},Fontface:{t:'字体'},FontSize:{t:'字体大小'},Bold:{t:'加粗 (Ctrl+B)',s:'Ctrl+B'},Italic:{t:'斜体 (Ctrl+I)',s:'Ctrl+I'},Underline:{t:'下划线 (Ctrl+U)',s:'Ctrl+U'},Strikethrough:{t:'删除线 (Ctrl+S)',s:'Ctrl+S'},FontColor:{t:'字体颜色'},BackColor:{t:'背景颜色'},Removeformat:{t:'删除文字格式'},Align:{t:'对齐'},List:{t:'列表'},Outdent:{t:'减少缩进 (Shift+Tab)',s:'Shift+Tab'},Indent:{t:'增加缩进 (Tab)',s:'Tab'},Link:{t:'超链接/传送文件'},Unlink:{t:'取消超链接'},Img:{t:'图片'},Flash:{t:'Flash动画'},Media:{t:'多媒体文件'},Emot:{t:'表情'},Table:{t:'表格'},Source:{t:'源代码'},Preview:{t:'预览'},Fullscreen:{t:'全屏编辑 (Esc)',s:'Esc'},About:{t:'关于 xhEditor'}};
var toolsThemes={
...
$.xheditor=function(textarea,options)
{
	...
	this.showEmot=function(group)
	{
		...
		group=group?group:(selEmotGroup?selEmotGroup:'qq');
	...
	this.uploadInit=function(jText,tourl,upext,setFunc)
	{
				...
				if(sFile!='')
				{
					if(sFile.match(new RegExp('\.('+upext.replace(/,/g,'|')+')$','i')))
					{
						...
						_this.ajaxUpload(jFile,tourl,function(data){
							...
							else setUploadUrl(data.msg,data);
		...
		function setUploadUrl(msg,data)//添加data参数,用于返回真实的文件名称 added by shixy 2010-5-13 17:00
		{
			...
			if(setFunc)setFunc(url);//自定义设置
				...
				if(data.title){//真实的文件名称
					$('#xheLinkText').val(data.title);
				}
				jText.val(url);//默认设置地址方法
			} 
