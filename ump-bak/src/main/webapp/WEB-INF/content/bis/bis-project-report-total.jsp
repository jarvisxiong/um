...
<div id="projRptDetail"></div>

<div style="margin-top: 10px; float: left;" id="buttons_group">
    <input type="button" value="查看本项目费用" class="btn_blue" style="width:105px; font-size:12px;"
           onclick="viewFeeManage();"/>
    <input type="button" value="查看本项目合同" class="btn_green" style="width:105px; font-size:12px;"
           onclick="viewContManage();"/>
</div>

<div id="projDebtPanel" style="border:0px solid #cccccc;">
    <%--
    <div class="res_tip">
...
<script type="text/javascript">
    function loadReportDetail() {
        ...
        var url = "${ctx}/bis/bis-project-report!loadReportDetail.action";
        $.post(url, data, function (result) {
            $("#projRptDetail").html(result);
        });
    }

    function loadProjectDebt() {
        var url = "${ctx}/bis/bis-project-report!loadProjectDebt.action";
        $.post(url, {bisProjectId:$("#bisProjectId").val()}, function (result) {
            $("#projDebtPanel").html(result);
        });
    }
</script>

<security:authorize ifAnyGranted="A_REPO_PROJ_QUERY">
    <script type="text/javascript">
        if (location.href.indexOf("debt") == -1) {
            loadReportDetail();
        }
    </script>
</security:authorize>

<security:authorize ifAnyGranted="A_REPO_ARRE_QUERY">
    <script type="text/javascript">
        //loadProjectDebt();
        if (location.href.indexOf("debt") != -1) {
            $("#buttons_group").hide();
