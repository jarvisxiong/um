...
<form id="nodeSearchForm" action="${ctx}/plan/cost-exec-plan-node!list.action" method="post">
	...
	<div  id="configNodeContentDiv" style="padding-left:5px; overflow: auto; overflow-x: hidden;">
	<table class="table_list" cellpadding="0" cellspacing="0" style="table-layout: fixed; width: 100%;">
		<thead>
			<tr class="header">
				...
				<th width="90" >是否控制节点</th>
				<th width="100" background: none;">操作</th>
			</tr>
		</thead>
		...
		<tbody>
			<tr id="newNodeAddedTr" style="display: none;" class="data">
				<td colspan="6">
					<table cellpadding="0" cellspacing="0" class="table_list" style="width:100%;">
						<tr class="data">
							<td style="width:40px;">
			...
			<s:iterator value="projNodeList" status="node">
				<tr class="data">
					<td style="cursor: pointer;" onclick="enableNodeCdEdit('${costProjectNodeRelId}');">
						<div id="${costProjectNodeRelId}NodeCd">${nodeCd}</div>
						<input onblur="saveNodeCd('${costProjectNodeRelId}');" id="${costProjectNodeRelId}NodeCdInput" type="text"
							name="${costProjectNodeRelId}nodeCd" value="${nodeCd}" style="width: 80%; display: none;" />
					</td>
					<td style="cursor: pointer;" onclick="enableNodeNameEdit('${costProjectNodeRelId}');" title="点击编辑节点名称">
						<div id="${costProjectNodeRelId}NodeName" title="${nodeName}">${nodeName}</div>
						<input onblur="saveNodeName('${costProjectNodeRelId}');" id="${costProjectNodeRelId}NodeNameInput" type="text"
							name="${costProjectNodeRelId}nodeName" value="${nodeName}" style="width: 95%; display: none;" />
					</td>
					<td align="left" style="cursor: pointer;" onclick="enableResOrgNameSel('${costProjectNodeRelId}');" title="点击编辑主责方">
						<div id="${costProjectNodeRelId}ResOrgName" class="pd-chill-tip" title="${resOrgName}">${resOrgName}</div>
					</td>
					<%--
					<td align="left" style="cursor: pointer;" onclick="enableCorOrgNameSel('${costProjectNodeRelId}');" title="点击编辑配合方">
						<div id="${costProjectNodeRelId}CorOrgName" class="pd-chill-tip" title="${corOrgName}">${corOrgName}</div>
					</td>
					--%>
					<td align="left" style="cursor: pointer;" onclick="enableOutputFilesEdit('${costProjectNodeRelId}');" title="点击编辑关键成果输出">
						<div id="${costProjectNodeRelId}OutputFiles" class="pd-chill-tip" title="${outputFiles}">${outputFiles}</div>
						<input onblur="saveOutputFiles('${costProjectNodeRelId}');" id="${costProjectNodeRelId}OutputFilesInput" type="text"
							name="${costProjectNodeRelId}outputFiles" value="${outputFiles}" style="width: 90%; display: none;" />
					</td>
					<td align="left" style="cursor: pointer;text-align: center;" onclick="enableControlNodeFlgSet('${costProjectNodeRelId}');" title="点击设置当前节点是否为控制计划节点">
						<div id="${costProjectNodeRelId}ControlNodeFlg">
							<s:if test="controlNodeFlg == 1">是</s:if>
							<s:else>否</s:else>
						</div>
						<select onblur="$(this).prev().show(); $(this).hide();" onclick="jQuery.Event(event).stopPropagation();" style="display: none;" onchange="saveControlNodeFlg('${costProjectNodeRelId}');"
							id="${costProjectNodeRelId}ControlNodeFlgSelect" name="${costProjectNodeRelId}controlNodeFlg">
					...
					<td id="${costProjectNodeRelId}OpsTd">
						<img style="cursor: pointer; margin-left: 10px;" title="删除节点"  src="${ctx}/pics/note/note_del.gif"
							onclick="deleteNode('${costProjectNodeRelId}');" />
					</td>
				</tr>
			</s:iterator>
		</tbody>
...
//注意:getCurProjectCd()引自 cost-exec-plan.js
	...
	function enableResOrgNameSel(nodeId) {
		...
		ymPrompt.confirmInfo({
			...
			afterShow:function(){
				$.post("${ctx}/plan/cost-exec-plan-node!buildOrgs.action",
						function(result) {
							if (result) {
								tree = new TreePanel({
									renderTo:'deptDiv',
	...
	function saveResOrgInfo(tp) {
		if (tp == "ok") {
			if (curNodeId && $.trim(curNodeId).length > 0) {
				if (confirm("确认修改负责方吗？")) {
					var url = "${ctx}/plan/cost-exec-plan-node!saveResOrgInfo.action";
	...
	function enableCorOrgNameSel(nodeId) {
		...
		ymPrompt.confirmInfo({
			...
			afterShow:function(){
				$.post("${ctx}/plan/cost-exec-plan-node!buildCheckOrgs.action",
						function(result) {
							if (result) {
								tree = new TreePanel({
									renderTo:'deptDiv',
	...
	function saveCorOrgInfo(tp) {
		if (tp == "ok") {
			...
			if (curNodeId && $.trim(curNodeId).length > 0) {
				if (confirm("确认修改配合方吗？")) {
					var url = "${ctx}/plan/cost-exec-plan-node!saveCorOrgInfo.action";
	...
	function deleteNode(nodeId) {
		...
		var url = "${ctx}/plan/cost-exec-plan-node!save.action";
		$.post(url,
				{
					id : nodeId,
					deletedFlg : "1",
		...
		$.post(url,
				{
					...
					nodeCd : nodeCd,
					nodeName : nodeName,
					planTypeCd : getPlanTypeCd(),//来源：cost-exec-plan.js
