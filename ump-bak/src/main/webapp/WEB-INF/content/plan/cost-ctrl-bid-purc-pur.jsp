...
<div id="new_templet" style="padding:5px;display: none; ">
	...
	<s:hidden name="pageType" id="add_page_type_id"></s:hidden>
...
<table class="content_table" id="result_table">
	...
	<tbody>
		<s:iterator value="dataMap" id="column" status="status">   
			...
			<!-- 主信息 -->
			<tr class="mainTr" id="main_${costCtrlBidPurcId}" group="group_<s:property value='#column.key'/>" 
				...
				<security:authorize ifAnyGranted="A_PUR_EDIT">
				...
				<security:authorize ifNotGranted="A_PUR_EDIT">
					permEdit="false"
				</security:authorize>
				<security:authorize ifAnyGranted="A_PUR_CONFIRM">
					<s:if test="scheduleStatusCd == 2">
						permConfirm="false"
					</s:if>
					<s:else>
				...
				<security:authorize ifNotGranted="A_PUR_CONFIRM">
							...
							<td  style="border-bottom:1px solid #DCDCDE;">
								<div style="height:28px;padding-top:5px;padding-left:10px;">
									...
									<s:if test="scheduleStatusCd == 0">
										<security:authorize ifAnyGranted="A_PUR_CONFIRM,A_PUR_EDIT">
									...
									<s:elseif test="scheduleStatusCd == 1">
										<security:authorize ifAnyGranted="A_PUR_CONFIRM,A_PUR_EDIT,A_PUR_SCHE">
										<button class="btn_green_55_20" style="margin-right:5px;" type="button" onclick="saveCost('${costCtrlBidPurcId}')">保存</button>
										</security:authorize>
										
										<!-- 去掉填写节点只有负责人能填写的限制
										<security:authorize ifNotGranted="A_PUR_CONFIRM,A_PUR_EDIT,A_PUR_SCHE">
											<s:if test="ownerCds.indexOf(currentUser+';')>-1">
											<button class="btn_green_55_20" style="margin-right:5px;" type="button" onclick="saveCost('${costCtrlBidPurcId}')">保存</button>
											</s:if>
										</security:authorize>
										 -->
									...
									<s:if test="scheduleStatusCd == 0 ||scheduleStatusCd == 1">
										<security:authorize ifAnyGranted="A_PUR_DELETE">
									...
									<table style="border:1px solid #DDDBDC">
										...
										<tbody>
											<s:iterator value="costCtrlSchedules" status="s" >   
												<tr
													<s:if test="scheduleTypeCd != 0 && scheduleTypeCd != 2">
												...
												>
													...
													<td>
														<input type="text" 
															...
															<security:authorize ifNotGranted="A_PUR_EDIT,A_PUR_CONFIRM,A_PUR_SCHE">
															disabled=disabled
															</security:authorize>
															<s:if test="scheduleTypeName == '定标'">
																id = "bid_date_${costCtrlBidPurcId}";
																<security:authorize ifAnyGranted="A_PUR_SCHE">
																	disabled=disabled
																</security:authorize>
															</s:if>
														/>
													...
													<td style="padding-left:5px;">
														...
														<security:authorize ifAnyGranted="A_PUR_EDIT,A_PUR_SCHE">
														<s:if test="#s.last">
															disabled=diabled
														</s:if>
														</security:authorize>
														...
														<%--去掉填写节点只有负责人能填写的限制
														<security:authorize ifNotGranted="A_PUR_EDIT">
															<s:if test="ownerCds == null || ownerCds == '' || ownerCds.indexOf(currentUser+';')<0">
															disabled=diabled
															</s:if>
															<s:else>
																<s:if test="#s.last">
													    ...
													    <span id="spanSup_${costCtrlBidPurcId}">
															<s:if test="scheduleTypeName=='投标单位确认'">
															 <s:if test="isSuppCompleteFlag==1">
															        ...
															        <security:authorize ifAnyGranted="A_PUR_EDIT,A_PUR_SCHE">
															        onclick="selectSuppliers('${costCtrlBidPurcId}','pur')"
															        </security:authorize>
															        />
															 </s:if>
															 ...
															 <s:else>
															 	 <security:authorize ifAnyGranted="A_PUR_EDIT,A_PUR_SCHE">
															        ...
															        <button class="btn_green_55_20" type="button" onclick="selectSuppliers('${costCtrlBidPurcId}','pur')">选择</button>
															        </span>
															        </security:authorize>
														        <div id="sup_have_${costCtrlBidPurcId}" style="display:none">
															          <img src="${ctx}/images/plan/pic_confirm.gif" title="已提供需求" style="cursor:pointer;"
																          onclick="selectSuppliers('${costCtrlBidPurcId}','pur')"
																        />
															        </div>
...
<div align="right" style="margin-top:10px;height:20px;width:100%;">
	...
	<script type="text/javascript">
		<security:authorize ifAnyGranted="A_PUR_ADD">
			$('#btn_add_cost').show();
		</security:authorize>
		<security:authorize ifNotGranted="A_PUR_ADD">
		$('#btn_add_cost').hide();
		</security:authorize>
	</script>