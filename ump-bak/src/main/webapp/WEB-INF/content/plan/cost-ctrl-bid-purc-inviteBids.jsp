							...
							<td  style="border-bottom:1px solid #DCDCDE;">
								<div style="height:28px;padding:5px;">
									...
									<s:if test="scheduleStatusCd == 0">
										<security:authorize ifAnyGranted="A_N_BP_CONFIRM,A_N_BP_EDIT">
									...
									<s:if test="scheduleStatusCd == 0 ||scheduleStatusCd == 1">
										<security:authorize ifAnyGranted="A_N_BP_DELETE">
										<input type="button" class="btn_new btn_del_new" onclick="deleteData('${costCtrlBidPurcId}')" value="删除" />
										</security:authorize>
									</s:if>
									
									<%-- 招标网批单 --%>
									<s:if test="inviteResId != null">
										<input type="button" class="btn_new btn_green_new" style="width:90px;" value="招标网批单" onclick="showPageLink('${ctx}/res/res-approve-info.action?id=${inviteResId}','resApprove');"/>
									</s:if>
									
									<%-- 定标网批单 --%>
									<span id="spanShowRes_${costCtrlBidPurcId}"> </span>
								    <s:if test="resApproveId != null">
								    	<input type="button" class="btn_new btn_green_new" style="width:80px;" value="定标审批表" onclick="showPageLink('${ctx}/res/res-approve-info.action?id=${resApproveId}','resApprove');" />
							        </s:if>
