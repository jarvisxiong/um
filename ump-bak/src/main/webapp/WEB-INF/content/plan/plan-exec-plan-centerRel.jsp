...
<head>
	...
	<link type="text/css" rel="stylesheet" href="${ctx}/resources/css/plan/plan-exec-plan-centerRel.css" />
	...
	<script type="text/javascript" src="${ctx}/resources/js/plan/plan-exec-plan-centerRel.js"></script>
		...
		<s:iterator value="#details.planDetailVO" var="detail">
		 <s:if test="!isExecSum&&planTypeCd==1&&!(preDelaySum==0&&preRecentSum==0)"><!-- isExecSum:有执行计划，但是在近一个月内没有项目 -->
			...
			<tr class="tr_${projectCd}" onclick="showPlanDetail('','${projNodeId}','${projPlanCd}','${detailId}');" 
				...
				<td><s:if test="nowStatus==0">过期</s:if> <s:elseif test="nowStatus==1">近期</s:elseif></td>
			</tr>
			<%
				orderNo++;
			%>
