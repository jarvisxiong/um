...
<script type="text/javascript">
	...
	opened_entityid = "${opened_entityid}";	//要打开的记录的id
 	if_in_attention = "${if_in_attention}";	//是否由关注的记录打开
 	nowDate = "<%=DateOperator.formatDate(DateOperator.truncDate(new Date()),"yyyy-MM-dd") %>";	//更新记录后显示更新时间用
 	page_size = 15; //分页大小，随列表区高度改变而改变
 	IF_DEPT_IN_SELECT = false;	//是否在部门列表内，如果不在，就不能新增
 	LIST_CENTER = new Array();	//中心的数组，给部门选择框用
 	if_in_center = false;	//是否在中心搜索内，如果在中心内则内容前面显示部门。refreshMain方法用
	if_chengben = false;	//是否成本，如果是要显示项目的选择框
	if_add_from_planExec = false;	//是否有新增来自执行计划的情况，如是则自动打开新增，填入相应信息
	
</script>
<s:set var="entityTmpId_New" ><%=RandomUtils.generateTmpEntityId() %></s:set>
<form id="newScheForm" method="post">
	<input type="hidden" id="pagePageNo" name="page.pageNo" value="${page.pageNo}"/>
...
<div id="costCtrlListDiv">
<table class="content_table" id="scheTable">
	<thead>
		<tr>
			<th align="left" width="80px;" style="background:none;">
			<input type="button" class="button_blue" onclick="newSchedule();" value="新增" />
			</th>
	...
	<tbody>
		...
		<tr id="detail_${planWorkCenterId}" class="detailTr" style="display:none;">
			<td <s:if test="centerCd=='11'">colspan="12"</s:if><c:if test="${if_chengben=='true'}">colspan="11"</c:if> colspan="10">
...
function saveMessage(planWorkCenterId){
	...
	function jumpTo(pageNo) {
		...
		refreshPlanWorkCenterList(pageNo);
	}
	function refreshPlanWorkCenterList(pageNo){
		$("#costCtrlListDiv").addClass("waiting").html("");
		$.post("${ctx}/plan/plan-work-center!list.action?search_orgCd=17&costModule=costCtrl&page.pageNo="+pageNo,
				function(result){
					$("#costCtrlListDiv").html(result);
		});
	}
</script>
