	...
	<form action="res-approve-info!save.action" method="post" id="templetForm" class="contractPaymentBill">
		...
		<table  class="mainTable">
			...
			<tr>
				...
				<td colspan="2">
					<input validate="required" type="text" name="templateBean.projectName" value="${templateBean.projectName}" id="projectName" readonly="readonly" class="inputBorderNoTip"/>
			...
			<tr>
				...
				<td colspan="3">
				<input id="engineeringName" class="inputBorder" validate="required" type="text" id="engineeringName" readonly="readonly" name="templateBean.engineeringName" value="${templateBean.engineeringName}" />
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td id="approveContent" colspan="1" class="chkGroup" align="left"  validate="required">
				...
				<input type="hidden" id="sid" name="templateBean.supBasicId" value="${templateBean.supBasicId}">
					...
					<s:else>
					<input id="unitName" class="inputBorder" validate="required" type="text" name="templateBean.unitName" id="unitName" value="${templateBean.unitName}" />
					</s:else>
				</td>
			</tr>
			...
			<tr>
				...
				<td colspan="3" style="padding-top:3px;">
					<table border="0" cellpadding="0" cellspacing="0" class="tb_noborder">
						...
						<td>招标范围:</td>
						<td>
						<input id="bidRange" class="inputBorder" validate="required" type="text" name="templateBean.bidRange" value="${templateBean.bidRange}" />
						</td>
						</tr>
					</table>
				</td>
			...
			<tr>
				<td colspan="3" style="padding-top:3px;">
					<table border="0" cellpadding="0" cellspacing="0" class="tb_noborder">
						...
						<col>
						<tr>
						<td><s:checkbox name="templateBean.inviteBidModel1"  cssClass="group"></s:checkbox>邀请招标单</td>
						</tr>
						<tr>
						<td><s:checkbox name="templateBean.inviteBidModel2"  cssClass="group"></s:checkbox>竞争性谈判</td>
						</tr>
						<tr>
						<td><s:checkbox name="templateBean.inviteBidModel3"  cssClass="group"></s:checkbox>直接委托(战略合作单位)</td>
						</tr>
						</table>
						</td>
						</tr>
			...
			<tr>
				<td colspan="3" style="padding-top:3px;">
					...
					<table border="0" cellpadding="0" cellspacing="0" class="tb_noborder">
						...
						<tr>
						<td><s:checkbox name="templateBean.pricingModel2"  cssClass="group"></s:checkbox>单价包干（按当地政府测绘部门最终确认的各业态建筑面积，按实结算）</td>
						</tr>
						</table>
						</td>
						</tr>
			...
			<tr>
				<td colspan="3" style="padding-top:3px;">
					<table border="0" cellpadding="0" cellspacing="0" class="tb_noborder">
						...
						<td>预算金额(元):</td>
						<td>
						<input class="inputBorder" validate="required" id="budgetAmount" type="text" name="templateBean.budgetAmount" value="${templateBean.budgetAmount}" onblur="formatVal($(this));" />
						</td>
						</tr>
					</table>
				</td>
			...
			<tr>
				<td colspan="3" style="padding-top:3px;">
					<table border="0" cellpadding="0" cellspacing="0" class="tb_noborder">
						...
						<td>付款方式:</td>
						<td>
						<input id="paymentType" class="inputBorder" validate="required" id="paymentType" type="text" name="templateBean.paymentType" value="${templateBean.paymentType}" />
						</td>
						</tr>
					</table>
				</td>
			...
			<tr  id="bidApprTrCont" style="display: none;">
			 <td style="height:40px;">定标审批表</td>
		    	<td></td>
		    	<td id="bidApprTdCont"></td>
			</tr>
			<tr id="bidApprTr" otype="attach">
				...
				<td>
				<input type="hidden" name="templateBean.bidApprIdFromCont" id="bidApprIdFromCont" value="${templateBean.bidApprIdFromCont}"/>
				<input type="hidden" name="templateBean.bidApprNoFromCont" id="bidApprNoFromCont" value="${templateBean.bidApprNoFromCont}"/>
			...
			<tr otype="attach">
				<td style="height:40px;" validate="required" value="${templateBean.contractItemApproveId}" resattachname="设计任务书">设计任务书</td>
				<td>
				<s:if test="#canEdit=='true'">
...
var mapPrama={
		...
		bidApprIdFromCont:'bidApprIdFromCont'
	};
<s:if test="authTypeCd=='ZCGL_HTQS_15'">
...
showBidAppr();
$("#contNo").quickSearch("${ctx}/cont/cont-ledger!quickSearch.action",['contNo','contName'],mapPrama,param,showModel,'','',showBidAppr);
...
function showBidAppr(){
	//如果定标审批表有值，则显示定标审批表链接
	var bidApprIdFromCont=$("#bidApprIdFromCont").val();
	if(""!=bidApprIdFromCont&&null!=bidApprIdFromCont){
		var showHtml="<span id='bidApprSpan' class='link'>"+$("#bidApprNoFromCont").val()+" </span>";
		$("#bidApprTdCont").html(showHtml);
		// 添加定标审批表链接
		$("#bidApprSpan").click(function(){
			var url=_ctx+"/res/res-approve-info.action?id="+bidApprIdFromCont;
			 if(parent.TabUtils==null)
					window.open(url);
				else
				 parent.TabUtils.newTab("bidApprId","审批查看",url,true);
		});
		$("#bidApprTrCont").show();
		$("#bidApprTr").hide();
	}else{
...
function chooseDisplay() {
	...
	if(noProjectFlg){
		$("#engineeringName").removeAttr("validate").removeClass('required');
		$("#approveContent").removeAttr("validate").removeClass('required');
		$("#unitName").removeAttr("validate").removeClass('required');
		$("#budgetAmount").removeAttr("validate").removeClass('required');
		$("#paymentType").removeAttr("validate").removeClass('required');
	}else{
		$('#engineeringName').attr("validate", "required").addClass('required');
		$('#approveContent').attr("validate", "required").addClass('required');
		$('#unitName').attr("validate", "required").addClass('required');
		$('#budgetAmount').attr("validate", "required").addClass('required');
		$('#paymentType').attr("validate", "required").addClass('required');
	}
}
...
function bindEvent(){
/*
	$("#contractNo").quickSearch("${ctx}/sc/sc-contract-templet-info!quickSearch.action",['contractName','contractNo'],ids,{sn:"QQ"},function(e){
		var $s = $(e);
...
/*
	...
	$("#contNo").bind("focus",function(){
	if($("#budgetAmount").attr("readonly")){
		if(isBind){
			unbindEvent();
	...
	$("#contractNo").bind("focus",function(){
...
function unbindEvent(){
	//$("#contNo").unbind('keyup').unbind("paste").unbind("keydown");
	//$("#contractNo").unbind('keyup').unbind("paste").unbind("keydown");
	isBind=false;
}
