			...
			<table style="width:100%;" >
				<tr>
					...
					<td align="right" valign="middle">
							...
							<s:if test="appModuleMenuRelId != null && appModuleMenuRelId != ''">
	...
	<tr>
		<td align="left" valign="top">
		 <s:form id="inputForm" action="app-module-menu-rel!save.action" method="post">
			    <!-- very important! -->
			    <input type="hidden" name="id" value="${appModuleMenuRelId}" />
			    <input type="hidden" name="menuCd" value="${menuCd}" />
			    <input type="hidden" id="entityRealId" name=appMenuId value="${appMenu.appMenuId}" />
				...
				<table class="mainTable" >
					...
					<tr>
						<td><s:text name="appAppModule.moduleName"/>:</td>
						<td>
							<!-- very important! -->
					...
					<tr>
						...
						<td>${appMenu.menuCd}</td>
						<td><div id="menuCdTip" ></div></td>
					</tr>
					
					<tr>
						<td><s:text name="appAppMenu.menuName"/>:</td>
						<td><s:textfield key="appMenu.menuName" id="menuName" name="appMenu.menuName" size="40"/></td>
						<td><div id="menuNameTip" ></div></td>
					</tr>
					
					<tr>
						<td><s:text name="appAppMenu.menuTypeCd"/>:</td>
						<td><s:radio list="mapAppMenuType" id="menuTypeCd" name="menuTypeCd" listKey="key" listValue="value" value="menuTypeCd==null?1:menuTypeCd"/></td>
						<td><div id="menuTypeCdTip" ></div></td>
					</tr>
					<tr>
						<td><s:text name="appAppMenu.menuTip"/>:</td>
						<td><s:textfield key="appMenu.menuTip" id="menuTip" name="appMenu.menuTip" size="40"/></td>
						<td><div id="menuTipTip" ></div></td>
					</tr>
					<tr>
						<td><s:text name="appAppMenu.dispOrderNo"/>:</td>
						<td><s:textfield key="appMenu.dispOrderNo" id="dispOrderNo" name="appMenu.dispOrderNo" size="40"/></td>
					...
					<tr>
						...
						<td>
							...
							<td>
								<!-- very important! -->
								<!-- if appModuleId is null, display drop down list, else display name -->
								<s:select list="mapAppPage" listKey="key" listValue="value" id="appPageId" name="appPageId" value="appMenu.appPage.appPageId"  onchange="changePageValue()"/>
							</td>
							<%--
							<td>
					...
					<tr>
						...
						<td><s:radio list="mapEnabledFlg" id="leafFlg" name="appMenu.leafFlg" listKey="key" listValue="value" value="appMenu.leafFlg==null?0:appMenu.leafFlg"/></td>
						<td><div id="leafFlgTip" ></div></td>
					</tr>
					<tr>
						<td>背景色加红
							<%-- 
							<s:text name="appAppMenu.fixFlg"/>
							--%>:</td>
						<td><s:radio list="mapEnabledFlg" id="fixFlg" name="appMenu.fixFlg" listKey="key" listValue="value" value="appMenu.fixFlg==null?0:appMenu.fixFlg"/></td>
						<td><div id="fixFlgTip" ></div></td>
					</tr>
					<tr>
						<td><s:text name="appAppMenu.sameWinFlg"/>:</td>
						<td><s:radio list="mapEnabledFlg" id="sameWinFlg" name="appMenu.sameWinFlg" listKey="key" listValue="value" value="appMenu.sameWinFlg==null?0:appMenu.sameWinFlg"/></td>
					...
					<tr>
						...
						<td><input type="text" id="parentMenuName" name="parentMenuName" value="${parentMenuName}"  onkeyup = "getMenuList(this);"/>
							<s:hidden key="appMenu.parentMenuCd" id="parentMenuCd" name="appMenu.parentMenuCd"/>
						</td>
						<td><div id="parentMenuCdTip" ></div></td>
					</tr>
					<tr>
						<td valign="top">图片名称:</td>
						<td><s:textarea key="appMenu.remark" id="remark" name="appMenu.remark" /></td>
						<td><div id="remarkTip" ></div></td>
					</tr>	
					<tr>
						<td>新版菜单图片路径:</td>
						<td><input type="text" id="iconPath2" name="appMenu.iconPath2" value="${appMenu.iconPath2}" />
						</td>
						<td></td>
					</tr>
					<tr align="center">
...
<script type="text/javascript"> 
	...
	//是否叶子节点
	$("input:radio[name='appMenu.leafFlg']").formValidator({empty:true,tipid:"leafFlgTip",onshow:"请选择是否叶子节点",onfocus:"请选择是否叶子节点",oncorrect:"已选择"}).inputValidator({min:1,max:20,onerror:"未选择是否叶子节点,请确认"}).defaultPassed();

	//是否允许修改
	$("input:radio[name='appMenu.fixFlg']").formValidator({empty:true,tipid:"fixFlgTip",onshow:"请选择是否允许修改",onfocus:"请选择是否允许修改",oncorrect:"已选择"}).inputValidator({min:1,max:20,onerror:"未选择是否允许修改,请确认"}).defaultPassed();

	//是否在同一窗口弹出打开
	$("input:radio[name='appMenu.sameWinFlg']").formValidator({empty:true,tipid:"sameWinFlgTip",onshow:"请选择是否在同一窗口弹出打开",onfocus:"请选择是否在同一窗口弹出打开",oncorrect:"已选择"}).inputValidator({min:1,max:20,onerror:"未选择是否允许修改,请确认"}).defaultPassed();
	...
	function menuAdd(){
		...
		var url = '${ctx}/app/app-module-menu-rel!input.action';
	...
	function menuDelete(moduleId,menuId){
		if(window.confirm('<s:text name="common.confirmDeleteMenu" />')){
			...
			document.location= '${ctx}/app/app-module-menu-rel!delete.action?id=${appModuleMenuRelId}';
		}
	}
	...
	function showUploadDiv() {
		ymPrompt.confirmInfo({
			...
			afterShow:function(){
				var entityId = "${appMenu.appMenuId}";
				if (!entityId || jQuery.trim(entityId).length == 0) {
					entityId = $("#entityTmpId").val();
				}
				url = "${ctx}/app/app-module-menu-rel!uploadPhoto.action?bizEntityId=" + entityId;
				$.get(url,
					function(result){
			...
			if (jQuery.trim($("#photoFile").val()).length > 0) {
				$("#photoForm").ajaxSubmit(function(result) {
					if (result) {
						...
						$.get("${ctx}/app/app-module-menu-rel!fetchJSONPhotoInfo.action?id=" + entityId, function(json) {
							var j = eval("(" + json + ")");
