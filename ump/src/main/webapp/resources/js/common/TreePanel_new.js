 ...
 * @version 2.0
 * @Desc:去掉checkbox   ，选择节点text，设置check属性
 */
...
TreePanel_New = function(config){
    ...
    this.haveClickText =true; //自定义叶子节点事件，点击叶子节点是否能选中			设置为ture
    this.isExpendSelect= false;//是否扩展选中事件

};

TreePanel_New.prototype.delegateGetDelegateIcon = function(node){
	return '';
};
TreePanel_New.prototype.delegateOnMouseOverNode = function(elem, node){
	return '';
};
TreePanel_New.prototype.delegateOnMouseOutNode = function(elem, node){
	return '';
};
TreePanel_New.prototype.delegateOnDblClick = function(node){
	return '';
};
TreePanel_New.prototype.delegateOnSelect = function(node){
	return '';
};


TreePanel_New.prototype={
	initialize : function(config){
		...
		var node = new TreeNode_New(config.root,this);
...
TreeNode_New=function(attributes,observer) {
	...
	for(var i=0,j=children.length;i<j;i++){
		var node = new TreeNode_New(children[i],observer);
		this.appendChild(node);
	}
};


TreeNode_New.prototype={
	initEl : function(){
		this['html-element']={};
		...
		this['html-element']['text'].className='TreeNodeNew';
		...
		if(this.checked===false||this.observer.haveClickText==true){
			this['html-element']['checkbox'].style.display='none';
		}
		if("0"==this.attributes.id){
			this['html-element']['text'].className='TreeNodeByTitle';
		}
		//自定义
	...
	paintCheckboxImg : function(){
		...
		if(this['html-element']){
			this['html-element']['checkbox'].src = '';
			this.setCheckClass();
//			this['html-element']['checkbox'].src = ownerTree.getIcon(((checked==2)?'checkbox2':(checked==1)?'checkbox1':'checkbox0'));
	...
	setCheckClass : function(){
		...
		if(checked=='2'||checked=='1'){
			...
			this['html-element']['text'].style.color = "#fff";
		}else{
			this['html-element']['text'].style.backgroundColor = "#E4E7EC";
			this['html-element']['text'].style.color = "#333";
		}
	},
	paintText : function(){
		...
		var width = '160';
		if(text.length*12>160){
			width = text.length*12;
		}
		this['html-element']['text'].style.width=width+'px';
	...
	select : function(){
		...
		/*if(this.isSelect)
			...
			//alert(this['html-element']['text'].style.color);
			//this['html-element']['text'].style.color = "333333";
		}*/
