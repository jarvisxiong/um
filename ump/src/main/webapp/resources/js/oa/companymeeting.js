...
function refreshFileList(folderId) {
	...
	$("#rightContainer").html("<div><div class='loading'>载入中...</div></div>");
	$.post(_PATH + "/oa/company-meeting!fileList.action",
			{
			folderId: folderId
			},
			function(result) {
...
function addNewFolder() {
	...
	$("#rightContainer").html("<div><div class='loading'>载入中...</div></div>");
	$.get(_PATH + "/oa/company-meeting!inputFolder.action", function(result) {
		if (result) {
			$("#rightContainer").html(result);
		}
	});
...
function editFolder() {
	...
	$("#rightContainer").html("<div ><div class='loading'>载入中...</div></div>");
	$.get(_PATH + "/oa/company-meeting!inputFolder.action?folderId=" + editedId, function(result) {
		if (result) {
			$("#rightContainer").html(result);			
		}
	});
...
function getParent() {
	...
	ymPrompt.confirmInfo({
		icoCls:"",
		title:"选择上级目录",
		message:"<div id='parentFolderDiv'><div class='loading'>载入中...</div></div>",
		useSlide:true,
		winPos:"t",
		width:400,
		height:400,
		...
		afterShow:function(){
		$.get(_PATH + "/oa/company-meeting!loadFolderTree.action?folderId=" + folderId, function(result) {
...
function delFolder() {
	if (confirm("确认删除当前目录吗？") == true) {
		$("#inputFolderForm").attr("action", "company-meeting!delFolder.action").submit();
	}
}
// 返回主页面
function back() {
	$("#inputFolderForm").attr("action", "company-meeting.action").submit();
}
// 更新目录树
function refreshFolderTree() {
	$.post(_PATH + "/oa/company-meeting!loadFolderTree.action", function(result) {
		$("#foldersTree").html("");
		if ($.trim(result) != "") {
			...
			$("#foldersTree").html("<div class='tip_nofolder'>没有文件夹可选择</div>");
		}
	});
}
// 新增 会议相关内容
function addFile(folderId) {
	...
	ymPrompt.confirmInfo({
		icoCls:"",
		title:"新增 会议相关内容",
		message:"<div id='addFileDiv'><div class='loading'>载入中...</div></div>",
		useSlide:true,
		winPos:"t",
		width:350,
		height:400,
		...
		handler:saveFile,
		afterShow:function(){
			$.get(_PATH + "/oa/company-meeting!inputFile.action?folderId=" + folderId, function(result) {
...
function editFile(fileId) {
	...
	ymPrompt.confirmInfo({
		icoCls:"",
		title:"编辑 会议相关内容 信息",
		message:"<div id='editFileDiv'><div class='loading'>载入中...</div></div>",
		useSlide:true,
		winPos:"t",
		width:350,
		height:400,
		...
		handler:saveFile,
		afterShow:function(){
			$.get(_PATH + "/oa/company-meeting!inputFile.action?folderId=" + folderId + "&fileId=" + fileId, function(result) {
				if (result) {
...
function delFile(fileId, folderId) {
	if (confirm("确认删除该文件吗?")) {
		$.post(_PATH + "/oa/company-meeting!deleteFile.action?fileId=" + fileId, function(result) {
			if (result == "done") {
				refreshFileList(folderId);
			}
		});
...
function uploadAttach(fileId) {
	...
	$("#inputUploadAttForm").ajaxSubmit(function(result) {
		if (result) {
			var url = _PATH + "/oa/company-meeting!fetchAttachList.action?bizEntityId=" + fileId;
...
function deleteAttachment(attachId, bizEntityId) {
	...
	$.post(url,
		{
			id:				attachId,
			bizModuleCd: 	"companyMeeting",
			bizEntityId:	bizEntityId
		},
		function(result) {
			if (result) {
				var url = _PATH + "/oa/company-meeting!fetchAttachList.action?bizEntityId=" + bizEntityId;
				$.get(url, function(result) {
					if (result) {
						$("#inputAttachList").html(result);
					}
...
function jumpTo(folderId, pageNo) {
	...
	if (p && t) {
		...
		$.get(_PATH + "/oa/company-meeting!fileList.action?folderId=" + folderId + "&filePage.pageNo=" + pageNo, function(result) {
			if (result) {
				$("#rightContainer").html(result);
			}
		});
