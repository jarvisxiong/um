...
if(col.cssStyle){
	//td div样式
 var _603=col.cssStyle;
cell.attr("style",_603);
}//end
}else{
...
//begin qlb
var opts=null;
if($(ed.target).attr("class").indexOf("combobox-f")>-1){
	opts={};
	opts.multiple=$(ed.target).attr("isMultiple");
	opts.relationField=$(ed.target).attr("relationField");	
	opts.isValidSelVal=$(ed.target).attr("isValidSelVal"); 
	opts.canSelectVal=","+$(ed.target).attr("canSelectVal")+",";
 }
var _47f="";
if(opts!=null && opts.relationField){
	_47f=$(ed.target).next().find("input").val(); 
}else{
	_47f=ed.actions.getValue(ed.target);
}
//关联的字段
if(opts!=null && opts.relationField){
	var fields=opts.relationField.split(',');
	if(fields.length>0){
	var _values="";
	var comboValues=$(ed.target).parent().find(".combo-value");
	$.each(comboValues,function(i,comItem){
		if(i>0){			
			_values+=",";
		}
		_values+=$(comItem).val();		
	});
	var _oldText=row[fields[0]];
	if(fields.length>1){
		_oldText=row[fields[1]];
	 }
	//如果需要校验输入的值
	 if(opts.isValidSelVal && opts.canSelectVal  && _oldText!=_47f ){
		  var _newVals= _values.split(",");
		  var _isDiff=false;
		  for(var n=0;n<_newVals.length;n++){
			  if(opts.canSelectVal.indexOf(","+_values+",")<0){
				  _isDiff=true;
				  
			  }
			  
		  }
		 if(opts.canSelectVal.indexOf(","+_values+",")<0  || _isDiff){//如果combobox中输入的值不存在则清空
			 _47f="";
			 _values="";
			 $(ed.target).next().find("input").val(""); 
		 }
	 ...
	 if(fields.length>1){
		...
		//IDS
		row[fields[0]]=_values;	
	
	}
}
//end
if(row[_47e]!=_47f){
	if(opts== null || opts.relationField == undefined){
		row[_47e]=_47f;
		}
_47c=true;
_47d[_47e]=_47f;
...
var _4c3={datetext:{init:function(_486,_487){
	var _488=$("<input type=\"text\" onfocus=\"WdatePicker()\"  onchange=\"splitDate(this)\" ctr=\"dateCtrl\" class=\"datagrid-editable-input\">").appendTo(_486);
	return _488;
...
//begin qlb
if( opts.treeGridId){
	var row= $(opts.treeGridId).treegrid('getSelected');
	//关联的字段   初始化下拉列表框
	if(opts!=null && opts.relationField && row){
		var fields=opts.relationField.split(',');
		if(fields.length>1){
		
			//IDS
			selectedIds=row[fields[0]].split(',');
			$(_675).find(".combobox-item[value=]").remove();
			$(_674).parent().find(".combo-value").remove();
				for(var i=0;i<selectedIds.length;i++){		
					if($.trim(selectedIds[i])==""){
						continue;
					}
					$(_675).find(".combobox-item[value="+selectedIds[i]+"]").addClass("combobox-item-selected");
					
					var _0927d=$("<input type=\"hidden\" class=\"combo-value\"  value='"+selectedIds[i]+"'/>").appendTo($(_674).parent());
				}
...
//begin qlb 设置供选的值
if(opts.isValidSelVal){
$(_6c5).attr("canSelectVal",_canSelectVal).attr("isValidSelVal",opts.isValidSelVal); 
}
if(opts.relationField){
	$(_6c5).attr("relationField",opts.relationField);
	}
//end
...
if(opts.multiple){
	//begin qlb
	$(_6c5).attr("isMultiple",opts.multiple);
	//opts.isUseMyselfSet=false;
	//是否使用自己的多选值设置
		if(opts.isUseMyselfSet){
			//end qlb
			...
			item.removeClass("combobox-item-selected");
				...
				if(opts.isCallParam && opts.isCallParam.values ==undefined){
					opts.isCallParam.values=item.attr("value");
					opts.isCallParam.texts=item.text();
				}
			}		
			if(opts.callSelected){
				 window.setTimeout(opts.callSelected,0);
				}
			var _texts="";
			var _values="";
			$(_6c5).parent().find(".combo-value").remove();
			var _itmDivs=$(item).parent();
			$.each(_itmDivs.find("div.combobox-item-selected"),function(i,itemDiv){				
			var _0927d=$("<input type=\"hidden\" class=\"combo-value\" >").appendTo($(_6c5));
				$(_0927d).val($(itemDiv).attr("value"));
				if(i>0){
				_texts+=",";
				_values+=",";
				}
				_texts+=$(itemDiv).text();
				_values+=$(itemDiv).attr("value");
			});
			$(_6c5).parent().find("span input").val(_texts);
			
			//begin qlb
			/*if(opts.isCallParam){
				opts.isCallParam.values=_values;
				opts.isCallParam.texts=_texts;
			}*/
		}else{
			//系统原始代码	
			if(item.hasClass("combobox-item-selected")){
