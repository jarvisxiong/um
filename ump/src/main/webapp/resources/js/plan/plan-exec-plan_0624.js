...
function switchProject(srcEle, curProjCd) {
	...
	var checkTreeTypeCd = $('input[name=checkTreeTypeCd]:checked').val();
	//alert(checkTreeTypeCd);
	
	var url  = _PATH + "/plan/plan-exec-plan!planMatrix.action";
	var data = {
			projectCd : curProjCd,
			planTypeCd: planTypeCd,
			checkTreeTypeCd: checkTreeTypeCd,
		...
		*/
			...
			function (result) {
				if (result) {
					...
					$("td[id='" + key + "ScheStDateTd']").attr("label", j.id);
					$("td[id='" + key + "ScheEdDateTd']").attr("label", j.id);
					$("td[id='" + key + "RealEdDateTd']").attr("label", j.id);
					$("td[id='" + key + "StatusTd']").attr("label", j.id);
					$("p[id='" + key + "ScheStDateSpan']").html(j.scheduleStDate);
					$("p[id='" + key + "ScheEdDateSpan']").html(j.scheduleEdDate);
					
					/*
					$("#" + key + "ScheStDateTd").attr("label", j.id);
					$("#" + key + "ScheEdDateTd").attr("label", j.id);
					$("#" + key + "RealEdDateTd").attr("label", j.id);
					$("#" + key + "StatusTd").attr("label", j.id);
					$("#" + key + "ScheStDateSpan").html(j.scheduleStDate);
					...
					*/
					...
					$("td[id='" + key + "StatusTd']").html(html);

					/* 20100826 "项目管理员"点击"完成"后自动提醒"管理员"审核
					if (curRole == "pj" && oriStatus == "0" && newStatus == "1") {						
						$.post(_PATH + "/plan/plan-exec-plan!remindVerify.action", { planDetailId : oriPlanDetailid });
					}
					*/
				}
			});
...
function resetMatrixLayout() {
	...
	var dataPanelHeight = getCurDocHeight() - leftHeight - portalTopHeight-33;
...
//点击某节点，打开下属所有子节点
function doOpenTree(nodeId){
	if(null!=nodeId && ""!=nodeId){
		if($("#tree_tr_"+nodeId).attr("hasNoSub")==0){
			if($("#tree_tr_"+nodeId).attr("mySubDisplay")==1){
				//收起
				try{
					$(".js_tree_"+nodeId).each(function(){
						//遍历下属的节点
						var tempId = $(this).attr("myid");
						if(null!=tempId && ""!=tempId && tempId!=nodeId){
							//如果此节点有下属节点，全部收起
							if($(".js_tree_"+tempId).size()==0){
							}else{
								$(".js_tree_"+tempId).hide();
								$(".js_tree_sub_"+tempId).hide();
								$("#tree_tr_"+tempId).attr("mySubDisplay","0");
								$("#jian_"+tempId).hide();
								$("#jia_"+tempId).show();
							}
						}
				...
				$(".js_tree_"+nodeId).hide();
				$(".js_tree_sub_"+nodeId).hide();
				$("#tree_tr_"+nodeId).attr("mySubDisplay","0");
				$("#jian_"+nodeId).hide();
				$("#jia_"+nodeId).show();
			}else{
				//打开
				$(".js_tree_"+nodeId).show();
				$(".js_tree_sub_"+nodeId).show();
				$("#tree_tr_"+nodeId).attr("mySubDisplay","1");
				$("#jia_"+nodeId).hide();
				$("#jian_"+nodeId).show();
			}
		}
...
var arr_treeParentNode = new Array();
function refreshJiaJian(){
	for(var i=0;i<arr_treeParentNode.length;i++){
		var nodeId = arr_treeParentNode[i];
		if($(".js_tree_"+nodeId).size()==0){
			$("#jia_"+nodeId).hide();
			$("#jian_"+nodeId).show();
			$("#tree_tr_"+nodeId).attr("hasNoSub","1");
		}
	}
}

function onClickCheckTreeType(){
	switchProject(null, getCurProjectCd());
}
