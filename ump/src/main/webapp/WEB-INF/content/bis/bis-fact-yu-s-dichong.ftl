...
<div id="dichong">
    ...
    <script type="text/javascript">
        ...
        $("form#frmAddAdvance2 button:contains(保存)").click(function () {
            ...
            var errorMessage = ump.helper.validation("#dichong .required");
            if (errorMessage && errorMessage != "") {
                ...
                if (parseFloat(money.val()) > parseFloat("#{model.surplusMoney}")) {
                    ump.ui.prompt.error({message:"抵充金额不能大于可用金额！"});
                    money.trigger("focus");
                    return;
                } else if (parseFloat("#{model.surplusMoney}") < 1) {
                    ump.ui.prompt.error({message:"可用金额为 “#{model.surplusMoney}”，不能进行抵充！"});
                    money.trigger("focus");
                    return;
                }
                try {
