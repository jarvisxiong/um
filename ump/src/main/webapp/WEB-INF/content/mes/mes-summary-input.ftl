    ...
    $(".publish_write .readonly button").click(function () {
        ...
        if (text == "返回") {
            ...
            if ("上传决议" == text) {
                ...
                TB_removeMaskLayer();
                var errorMessage = ump.helper.validation(".rst .required");
                        ...
                        if (pd.helper.trim(element.attr("rel")) != "") {
                            $.getJSON("mes-meeting-info!ajax.action", {action:1001, id:element.attr("rel")}, function ($data) {
                                if ($data.success) {
                                    ...
                                    ump.ui.prompt.error({message:"Sorry 您只能删除自己上传的附件!", time:3000});
                                }
                                ymPrompt.close();
                            });
                            //远程删除
                ...
                if ($data.start && $data.start == "1") {
                    ...
                    $.getJSON("mes-meeting-info!ajax.action", {action:1004, "id":"${mesMeetingInfo.mesMeetingInfoId!""}"}, function ($input) {
                        if ($input.success) {
                            ...
                            ump.ui.prompt.error({message:$input.message});
                        }
                    });
                    ...
                    ump.ui.prompt.success({message:"操作成功，已于" + timer.getHours() + ":" + timer.getMinutes() + ":" + timer.getSeconds() + " 秒保存 。"});
                }
            });
        } else {
