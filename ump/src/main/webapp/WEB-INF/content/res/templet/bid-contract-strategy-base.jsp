...
<table class="mainTable" style="margin-bottom: 5px;">
	...
	<tr id="contlibno" style="display: none">
		...
		<input type="hidden" id="nonstandard" name="templateBean.nonstandard" value="${templateBean.nonstandard}" />
		 <s:if	test="!templateBean.contractTempletInfoId.isEmpty() && #canEdit == 'false'">
			<span class="link scContractLink" onclick="getContById('${templateBean.contractTempletInfoId}','${templateBean.contractTempletHisId}');" > ${templateBean.scContractNo}
			</span>
		...
		<s:else>
			<input class="inputBorder" id="scContractNo" validate="required" type="text" name="templateBean.scContractNo" value="${templateBean.scContractNo}" />
			<input type="hidden" id="contractTempletInfoId" name="templateBean.contractTempletInfoId" value="${templateBean.contractTempletInfoId}" />
			<input type="hidden" id="contractTempletHisId" name="templateBean.contractTempletHisId" value="${templateBean.contractTempletHisId}" />
		</s:else></td>
		<td>
...
<div id="scattachList" style="display: none;"></div>
</div>
<script>

var ids = {
	contractNo: 'scContractNo',
...
function callChooseContlibFun(flag){
	...
	function doContMark() {
		...
		if ($("#scContractNo").val() != "" && $("#contractTempletInfoId").val() != "") {
...
function autoQuickSearchCtledger(contNo){
	   ...
	   if(!isHasVal){//如果未关联合同台账则清除之前的数据
		   if(idctLedgerArray)
			...
			if($("#isJudageCtLedgerCons").val() == "1"){			
				if($("#contNo").val() != $("#scContractNo").val()){				
	...
	//编辑状态下才绑定搜索
	<s:if test="#canEdit == 'true'">
		$("#scContractNo").quickSearch("${ctx}/sc/sc-contract-templet-info!quickSearch.action", [ 'contractName', 'contractNo' ], ids, {}, function(e) {
			var $s = $(e);
			...
			if(typeof(autoRelationCtLedger) !="undefined"){
				autoQuickSearchCtledger($("#scContractNo").val());
