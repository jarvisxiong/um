	...
	<div class="bodyHead">
		...
		<div class="headContent">
			<div><font style="font-size: 14px;color: red;">正在查看:</font>
				...
				<select  id="curSelBidLedgerId" name="bidLedgerId"  style="width: 100px;" >
				    <s:iterator value="listBidLedgers" var="bl" status="status">
				    		<option value="${bidLedgerId}"  title="${bidSectionName}"
			...
			<select  id="bidProjectId" style="width: 90px;" name="bidProjectId">
			    <s:iterator value="listProjects" var="bl" status="status">
			    	<option value="${bidProjectId}" title="${projectName}">${projectName}</option>
			    </s:iterator>
			</select>
			</div>
			...
			<div style="float: right;" class="full_screen" onclick="fullScreen('${ctx}/bid/bid-analysis!list.action?bidLedgerId=${bidLedger.bidLedgerId}&orgCd=${bidLedger.orgCd}&new=1');" ><font style="padding-left: 15px;font-size: 14px;">全屏</font></div>
		</div>
		...
		<div class="tools">
			...
			<div class="bidPanel" style="margin-left: 5px;">
				...
				<ul id="bid-nav" class="bid-nav">
					...
					<li typeCd="1" enablesupflg="yes" nodeValue="quantityAnalysis" style="width: 100px;text-align: center;">量/价比</li>
					<li typeCd="2" nodeValue="priceTotal" style="width: 100px;text-align: center;">占总价比例</li>
					<li typeCd="3" nodeValue="perMeterAnalysis" style="width: 100px;text-align: center;">平方米指标</li>
					<li typeCd="4" nodeValue="bidTotalAnalysis"  style="width: 100px;text-align: center;">投标总价汇总</li>
					<li typeCd="5" nodeValue="bidDivisitAnaly"  style="width: 100px;text-align: center;">清单轮次差异</li>
					<li typeCd="6" nodeValue="bidProjectAnaly" enablesupflg="nosyes" style="width: 100px;text-align: center;">标段工程汇总</li>
				</ul>
			</div>
			...
			<div class="supPanel" style="display:none;margin-left: 5px;">
				...
				<div class="compareFieldsPanel" style="float:left;clear:left;margin-left: 0px;">
					<font  style="padding-left: 5px;font-size: 14px;">可选择列:
						...
						<input type="text" style="height: 20px;" readonly="readonly" id="itemNames" onclick="selectBidCountry();"/>国标选取
					</font>
				</div>
			</div>
			...
			<div class="batchNoPanel">
					...
					<div style="margin-top:1px;">
						...
						<div class="plblue" onclick="exportExecl();" id="exportBtn">导出</div>
...
<script language="javascript">
		...
		$('.bid-nav li').click(function(){
			if( 'nosel' != $(this).attr('class')){
				...
				if('yes' == $(this).attr('enablesupflg')){
					$('.supPanel').show();
					$('.compareFieldsPanel').show();
				}else{
					$('.supPanel').hide();
					if('nosyes'== $(this).attr('enablesupflg')){
						$('.supPanel').show();
						$('.compareFieldsPanel').hide();
					}
				}
	...
	function decorateCol(nodetype){
		$(".rightDetail tbody tr").each(function(){
			...
			if(toppest && lowest){		
				//循环比较 
				if((count-1) >= 6){
					avgVal = (totalVal - parseFloat(toppest.text()) - parseFloat(lowest.text()))/(count - 3);					
				}else{
					...
					if(tIndex != 0){
						if($(this).hasClass('top') || $(this).hasClass('low')){
							...
							if(t<=leftVal || t>=rightVal){
								var avgr=$(this).text();
								if(avgr!=toppest.text()&&avgr!=lowest.text())
			...
			//赋值算术平均值
			if('totalPrice'==nodetype)
			jqRow.find("[totalPriceAvg='1']").text(new Number(avgVal).toFixed(1));
			if('unitPrice'==nodetype)
			jqRow.find("[unitPriceAvg='1']").text(new Number(avgVal).toFixed(1));
	});
	}
	...
	function bidDivisitAnaly(){
		var selectedSupIds = getSelectedSup();
		var selectedBatchnos = getBatchNo();
		var bidLedgerId = getSelBidLedgerId();//'${bidLedger.bidLedgerId}';
		var bidProjectId=$('#bidProjectId').val();
		var url = _ctx+"/bid/bid-analysis!bidDivisitAnaly.action";
		var data = {
			bidLedgerId : bidLedgerId
		};
		if (selectedBatchnos) {
			selectedBatchnos = selectedBatchnos.substring(1,selectedBatchnos.length);
			data = {bidLedgerId : bidLedgerId,batchNo : selectedBatchnos,bidProjectId:bidProjectId};
			TB_showMaskLayer("正在分析...");
			$.post(url, data, function(result) {
				TB_removeMaskLayer();
				$('#compareResultPanel').html(result).show();
	...
	function bidProjectAnaly(){
		var selectedSupIds = getSelectedSup().substring(1, getSelectedSup().length);
		var selectedBatchnos = getBatchNo();
		var bidLedgerId = getSelBidLedgerId();//'${bidLedger.bidLedgerId}';
		var bidProjectId=$('#bidProjectId').val();
		var url = _ctx+"/bid/bid-analysis!bidProjectAnaly.action";
		var data = {
			bidLedgerId : bidLedgerId
		};
		if (selectedBatchnos) {
			selectedBatchnos = selectedBatchnos.substring(1,selectedBatchnos.length);
			data = {bidLedgerId : bidLedgerId,batchNo : selectedBatchnos,bidProjectId:bidProjectId,bidSupId:selectedSupIds};
			TB_showMaskLayer("正在分析...");
			$.post(url, data, function(result) {
				TB_removeMaskLayer();
				$('#compareResultPanel').html(result).show();
	...
	function submitSearch(){
		...
		switch(typeCd){
			...
			case '5':
				if(batchNoCnt !=2){
					showSuccessInfo("searchTip","请选择两个投标轮次!");
					return ;
				}
				//设置模块名
				setModuleName('清单轮次差异分析');
				bidDivisitAnaly();
				break;
			//标段工程汇总
			case '6':
				if(supCnt != 1){
					showSuccessInfo("searchTip","只能选择一家投标单位!");
					return;
				}
				if(batchNoCnt !=1){
					showSuccessInfo("searchTip","请选择一个投标轮次!");
					return ;
				}
				//设置模块名
				
				setModuleName('标段工程汇总');
				bidProjectAnaly();
				break;
		}
	...
	function selectBidCountry(){
		var bidProjectId=$('#bidProjectId').val();
		var itemIdsTemp=$('#itemIds').val();
		var itemNamesTemp=$('#itemNames').val();
		ymPrompt.win( {
			icoCls : "",
			autoClose:false,
			message : "<div id='bidCountryInputDiv'><img align='absMiddle' src='"
				...
				afterShow : function() {
					...
					$.post(url,{bidProjectId:bidProjectId,itemIdsTemp:itemIdsTemp,itemNamesTemp:itemNamesTemp}, function(result) {
						$("#bidCountryInputDiv").html(result);
					});
				},
				handler : function(btn){
					if(btn=='ok'){
						$('#itemIds').val($('#itemIdsTemp').val());
						$('#itemNames').val($('#itemNamesTemp').val());
					}
					ymPrompt.close();
				},
				btn:[["确定",'ok'],["退出",'cancel']]
	...
	function exportExecl(){
		...
		if(batchNoCnt>1){
			showSuccessInfo("searchTip","不能进行多轮次导出!");
			return ;	
			}
		var bidLedgerId = getSelBidLedgerId();
		var bidProjectId=$('#bidProjectId').val();
		var selectedBatchnos = getBatchNo();
		var searchType='sup';
		var supIds=getSelectedSup();
		if (selectedBatchnos)
			selectedBatchnos = selectedBatchnos.substring(1,selectedBatchnos.length);
		TB_showMaskLayer("正在导出...");
		window.location.href = _ctx + "/bid/bid-analysis!quantityPriceExcel.action?bidLedgerId=" + bidLedgerId + "&bidProjectId=" + bidProjectId+"&batchNo="+selectedBatchnos+"&searchType="+searchType+"&supIds="+supIds;
		TB_removeMaskLayer();
		}
</script>
</body>
