			...
			<div class="headTitle">投标单位信息(<s:property value="listBidSupVo.size"/>)</div>
				<security:authorize ifAnyGranted="A_BID_BASIC_EDIT,A_ADMIN_BID">
					<s:if test="bidLedger.bidStatusCd<1">	       		   	  	
						<input type="button" class="btn_new btn_add_new" id="btnAddSup" value="新增" />
					</s:if>
				</security:authorize>	       		   	  	
							...
							<tr>
	         	            ...
	         	            <tr>
	         	            	...
	         	            	<td>
	         	            		...
	         	            		<input type="button" class="btn_new btn_cancel_new" onclick="$('#btnAddSup').trigger('click');" value="取消" />
	         	            	</td>
	         	            </tr>
				         	</security:authorize> 
	        	    	</table>
			...
			<thead>
				...
				<s:iterator value="listBidSupVo" var="vo" status="status" >
					...
					<tr class="supBrief" id="supBrief_${bidSupId}" title='点击查看明细'>
		  			  	...
		  			  	<%--如果是暗标则不显示供应商名称 --%>
						<td  <s:if test="#status.index==listBidSupVo.size()-1"> class='lastRowTd' </s:if> title='<s:if test="bidVisibleFlg==1">${bidSupName}</s:if><s:else>×××</s:else>'>
						...
						<td  <s:if test="#status.index==listBidSupVo.size()-1"> class='lastRowTd' </s:if>><div class="partHide ellipsisDiv_full">
							<s:if test="bidVisibleFlg == 1">								
								<%--如果在邀标阶段前 --%>
									...
									<%--供应商 --%>
									<s:if test="typeCd==1">
										<div id="linkman_${bidSupId}" <s:if test="bidLedger.bidStatusCd<1"><security:authorize ifAnyGranted="A_BID_BASIC_EDIT,A_ADMIN_BID">onclick="changeCont(this,'${refBidSupId}','${bidSupId}');"</security:authorize></s:if>>								
						...
						<td  <s:if test="#status.index==listBidSupVo.size()-1"> class='lastRowTd' </s:if>><span><p:code2name mapCodeName="@com.hhz.ump.util.DictMapUtil@getMapSupBasicEvaluate()" value="supLevelCd"/></span></td>
						<td  <s:if test="#status.index==listBidSupVo.size()-1"> class='lastRowTd' </s:if>><span>${totalBatchCount}</span></td>
						...
						<s:if test="bidLedger.bidStatusCd<1&&typeCd==1">
							<td>
								<input type="button" class="btn_new btn_del_new" onclick="doDeleteSup('${bidSupId}','${bidLedger.bidLedgerId}');" value="删除"/>
							</td>
						</s:if>
					</tr>
					<%-- 显示编辑表单部分 --%>
					...
					<td class="hidden_sub_panel" style="vertical-align: top;">
						 <s:if test="receiveStatusCd == 1">
							...
							--%>
							<s:if test="typeCd==1">
								<s:if test="bidLedger.bidStatusCd == 4&&supBidStatusCd == 1">
									<input type="button" class="btn_new btn_cancel_new" onclick="cancelBidSucc('${bidSupId}','${bidLedgerId }')" style="width:70px" value="取消推荐中标"/>
								</s:if>
								<s:if test="bidLedger.bidStatusCd == 4&&supBidStatusCd != 1">
									<input type="buttoon" class="btn_new btn_blue_new" onclick="setBidSucc('${bidSupId}','${bidLedgerId }')" style="width:70px" value="推荐中标" />
								</s:if>
							</s:if>
						</s:if>
					</td>
