...
<script type="text/javascript">
	...
	if (father != null) {
		...
		$("form :input[type!=hidden]").filter("[type!='checkbox']").each(function(i){
			var isNone=$(this).css('display');
			if (isNone!='none'){
			var ori_value=$(this).val();
			$(this).replaceWith('<span style="text-align:left;">'+ori_value+'</span>');
			}
		});
		$("form pre").each(function(i){
			var p=$(this).parent();
			var ori_value=$(this).html();
			p.replaceWith('<div style="text-align:left;">'+ori_value+'</div>');
			//$(this).replaceWith(ori_value);
			/*
			var _td=$(this).parents("td:first");
			if (_td.length==1){
				_td.html($(this).html());
			}
			*/
	...
	function loadVersion(){
		$.post(_ctx+"/app/app-print-his!load.action",{
