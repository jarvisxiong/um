...
public class BidTmpImportDataManager extends BaseService<BidTmpImportData, String> {
	...
	public void validateTmp(String userId, String batchNo, String bidStatus) {
		...
		try {
			...
			map.put(3, bidStatus);
			// --分部分项（标底公司）
			excuteProc("{ call ump.bid_temp_pkg.validateBidDivision(?,?,?,?)}", map, conn);

			// --分部分项（投标公司）
			map.remove(0);
			map.put(0, "BidDivisionSupRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidDivisionSuperRel(?,?,?,?)}", map, conn);

			// --措施1
			map.remove(0);
			map.put(0, "BidMeasureSupRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidMeasureSupRel(?,?,?,?)}", map, conn);

			// --措施2
			map.remove(0);
			map.put(0, "BidMeasureSupRel2");
			excuteProc("{ call ump.bid_temp_pkg.validateBidMeasureSupRel2(?,?,?,?)}", map, conn);

			// --其他项目清单与计价汇总表
			map.remove(0);
			map.put(0, "BidOtherItemRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidOtherItemRel(?,?,?,?)}", map, conn);

			// --零星工程费用表
			map.remove(0);
			map.put(0, "BidSporadicSupRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidSporadicSupRel(?,?,?,?)}", map, conn);

			// --规费项目清单与计价表
			map.remove(0);
			map.put(0, "BidFeesSupRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidFeesSupRel(?,?,?,?)}", map, conn);

			// --税金项目清单与计价表
			map.remove(0);
			map.put(0, "BidTaxsSupRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidTaxsSupRel(?,?,?,?)}", map, conn);

			// --标段其他费用
			map.remove(0);
			map.put(0, "BidSupOtherRel");
			excuteProc("{ call ump.bid_temp_pkg.validateBidSupOtherRel(?,?,?,?)}", map, conn);
		} catch (Exception e) {
			log.error(e);
		} finally {
			try {
		...
		try {
			...
			if ("1".equals(qingdan)) {
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())) {
					...
					// }
					// 导入清单
					excuteProc("{ call ump.bid_temp_pkg.importBidDivisiton(?,?,?)}", map, conn);
				}
				flag = true;
			}
			// 如果非导入清单数据
				...
				if (this.getCntByTableName("BidDivisionSupRel", currentUiid, bachId, bidSupId, bidProjectId) > 0) {
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidDivisionSupRel(?,?,?)}", map, conn);
				}
				// 导入增添项(2012-6-25)
				if (this.getCntByTableName("BidDivisitonCust", currentUiid, bachId, bidSupId, bidProjectId) > 0) {
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidDivisitonCust(?,?,?)}", map, conn);
				}
				// --导入措施1
				// 有新数据需要导入、投标状态
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidMeasureSupRel(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidMeasureSupRel2(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidOtherItemRel(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidSporadicSupRel(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidFeesSupRel(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidTaxsSupRel(?,?,?)}", map, conn);
				}
				...
				if (BidLedgerManager.BID_STATUS_BIDDING.equals(bidLedger.getBidStatusCd())
					...
					session.flush();
					excuteProc("{ call ump.bid_temp_pkg.importBidSupOtherRel(?,?,?)}", map, conn);
				}
