...
@Service
@Transactional
public class PlanExecPlanDetailOldManager extends BaseService<PlanExecPlanDetailOld, String> {

	private static final Logger logger = Logger.getLogger(PlanExecPlanDetailOldManager.class);
	private static final String MAIL_ADMIN = "email_admin";
	public static final String MAIL_TYPE_VERIFY = "verify";

	@Autowired
	private PlanExecPlanDetailOldDao planExecPlanDetailDao;
	@Autowired
	private PlanProjectNodeRelOldManager nodeRelManager;
	@Autowired
	...
	// PlanExecPlanDetailOld Manager //
	@Transactional(readOnly = true)
	public PlanExecPlanDetailOld getPlanExecPlanDetail(String id) {
		return planExecPlanDetailDao.get(id);
	}

	public List<PlanExecPlanDetailOld> getAllPlanExecPlanDetail() {
		return planExecPlanDetailDao.getAll();
	}

	public void savePlanExecPlanDetail(PlanExecPlanDetailOld planExecPlanDetail) {
	...
	@Override
	public HibernateDao<PlanExecPlanDetailOld, String> getDao() {
	 ...
	 * @return
	 */
	public List<PlanExecPlanDetailOld> getDetailsByProjPlanNodes(String[] nodeIds){
		return getDetailsByProjPlanNodes(nodeIds,null);
	}
	public List<PlanExecPlanDetailOld> getDetailsByProjPlanNodes(String[] nodeIds,String treeTypeCd) {
		
		if (nodeIds == null || nodeIds.length == 0)
			return new ArrayList<PlanExecPlanDetailOld>();
		else {
			Map<String, Object> params = new HashMap<String, Object>();
			StringBuilder sb = new StringBuilder("from PlanExecPlanDetailOld where");

			if(StringUtils.isNotBlank(treeTypeCd)){
				sb.append(" planProjectNodeRel.treeType <= :treeType and ");
	 ...
	 * @throws Exception 
	 */
	public void addPlanDetailInfo(String scheduleStartDate, String scheduleEndDate, String realEndDate, String planDetailTempId,
			String projNodeId, String projPlanCd) throws Exception {

		PlanExecPlanDetailOld detail = new PlanExecPlanDetailOld();
		PlanProjectNodeRelOld nodeRel = nodeRelManager.getEntity(projNodeId);
		...
		String operationType = PlanOperationLogOldManager.OPERATION_TYPE_ADD;
		...
		if (attachList.size() > 0) {
			...
			operationType = PlanOperationLogOldManager.OPERATION_TYPE_UPLOADATTACH;
	...
	public void svPlanDetailInfo(String scheduleStartDate, String scheduleEndDate, String realEndDate, String planDetailId, String projNodeId,
		...
		PlanExecPlanDetailOld detail = this.getEntity(planDetailId);
		...
		String nodeName = planNodeManager.getNodeNameByCd(nodeRel.getNodeCd());
		String planName = planManager.getPlanNameByPlanCd(projPlanCd);
		String operationType = PlanOperationLogOldManager.OPERATION_TYPE_UPDATE;
		String operationLog = "";
	...
	public boolean updatePlanDetailStatus(String planDetailId, String newStatus) throws Exception {
		...
		Map<String, String> statusMap = this.buildPlanDetailStatusMap();
		PlanProjectNodeRelOld nodeRel = detail.getPlanProjectNodeRel();
		String nodeName = planNodeManager.getNodeNameByCd(nodeRel.getNodeCd());
		String planName = planManager.getPlanNameByPlanCd(detail.getExecutionPlanCd());
		PlanExecutionPlanMainOld planMain = nodeRel.getPlanExecutionPlanMain();
		...
		operationLogManager.addPlanLog(detail.getPlanProjectNodeRel().getPlanTypeCd(), planDetailId, nodeName + "-" + planName,
				PlanOperationLogOldManager.OPERATION_TYPE_UPDATE, projectCd, operationLog);
	 ...
	 * @param mailType
	 */
	private void sendRemindEmail(PlanExecPlanDetailOld planDetail, String mailType) {
		PlanProjectNodeRelOld nodeRel = planDetail.getPlanProjectNodeRel();
		String nodeName = planNodeManager.getNodeNameByCd(nodeRel.getNodeCd());
	 ...
	 * @return
	 */
	private String buildMailBody(PlanExecPlanDetailOld planDetail, String mailType) {
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		Map<String, String> statusMap = this.buildPlanDetailStatusMap();
	 ...
	 * @return
	 */
	public PlanExecPlanDetailOld getPlanDetailPerNodeRelIdAndPlanCd(String nodeRelId, String planCd) {
		String hql = "from PlanExecPlanDetailOld where planProjectNodeRel.planProjectNodeRelId = :nodeRelId and executionPlanCd = :planCd";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("nodeRelId", nodeRelId);
		params.put("planCd", planCd);
		List<PlanExecPlanDetailOld> result = this.find(hql, params);
	...
	public boolean confirmPlanDetailInfo(String planDetailId) throws Exception {
		...
		PlanExecPlanDetailOld pd = this.getEntity(planDetailId);
		pd.setInfoConfirmedFlg("1");
		savePlanExecPlanDetail(pd);

		PlanProjectNodeRelOld nodeRel = pd.getPlanProjectNodeRel();
		String nodeName = planNodeManager.getNodeNameByCd(nodeRel.getNodeCd());
	...
	public boolean batchConfirmPlanDetailInfo(String projectCd, String planTypeCd) {
		...
		String hql = new StringBuffer()
					.append(" update PlanExecPlanDetailOld pd ")
					...
					.append("          from PlanProjectNodeRelOld ")
	...
	public Map<String, String> buildPlanDetailStatusMap() {
		...
		m.put(DictContants.EXEC_PLAN_DETAIL_STATUS_CONFIRM_COMPLETED, "审核确认");
		return m;
	}
	...
	public void daylyRemind() {
		List<PlanExecPlanDetailOld> plans = findPlansShouldBeReminded();
		for (PlanExecPlanDetailOld p : plans) {
	 ...
	 * @return
	 */
	public List<PlanExecPlanDetailOld> findPlansShouldBeReminded() {
		Date today = DateOperator.truncDate(new Date());
		Date twoWeeksLater = DateOperator.addDays(today, 14);

		String hql = "from PlanExecPlanDetailOld pd where pd.status != 2 and pd.scheduleEndDate <= :twoWeeksLater";
		Map<String, Object> params = new HashMap<String, Object>();
		params.put("twoWeeksLater", twoWeeksLater);
		List<PlanExecPlanDetailOld> plans = this.find(hql, params);
		List<PlanExecPlanDetailOld> result = new ArrayList<PlanExecPlanDetailOld>();
	 ...
	 * @return
	 */
	public List<PlanExecPlanDetailOld> getSortedStatPlanNodeDetail(String projectCd,String planTypeCd,String executionPlanCd) {
		if (StringUtils.isBlank(projectCd))
			return new ArrayList<PlanExecPlanDetailOld>();

		String hql = new StringBuffer()
					.append("from PlanExecPlanDetailOld d ")
		...
		params.put("planTypeCd", planTypeCd);
		params.put("controlNodeFlg", PlanProjectNodeRelOldManager.CONTROL_NODE_FLG_ENABLE);
		params.put("notCompleted", DictContants.EXEC_PLAN_DETAIL_STATUS_NOT_COMPLETED);
		params.put("deletedFlg", PlanProjectNodeRelOldManager.DELETED_FLG_Y);
		params.put("currentDate",  java.sql.Date.valueOf(DateOperator.formatDate(new Date(),"yyyy-MM-dd")));
		

		List<PlanExecPlanDetailOld> list = getDao().find(hql, params);
		Collections.sort(list, new Comparator<PlanExecPlanDetailOld>() {
			@Override
			public int compare(PlanExecPlanDetailOld n1, PlanExecPlanDetailOld n2) {
	...
	public Map<String, String> getDetailBriefMap(String planDetailId){
		...
		PlanExecPlanDetailOld detail = getEntity(planDetailId);
		if(detail == null)
			return returnMap;
	...
	public List<Object> getStatData(String yearMonth) {
		...
		.append("     from plan_exec_plan_detail_old t ,plan_project_node_rel_old t2, plan_execution_plan_main_old t3 ")
