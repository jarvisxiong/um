...
@Service
@Transactional
public class PlanExecutionPlanMainOldManager extends BaseService<PlanExecutionPlanMainOld, String> {

	private Logger logger = Logger.getLogger(PlanExecutionPlanMainOldManager.class);

	@Autowired
	private PlanExecutionPlanMainOldDao planExecutionPlanMainDao;
	@Autowired
	private PlanProjectNodeRelOldManager projNodeRelManager;
	@Autowired
	private PlanExecutionPlanNodeOldManager execPlanNodeManager;
	@Autowired
	private PlanOperationLogOldManager planOperationLogManager;

	@Transactional(readOnly = true)
	public PlanExecutionPlanMainOld getPlanExecutionPlanMain(String id) {
		return planExecutionPlanMainDao.get(id);
	}

	public List<PlanExecutionPlanMainOld> getAllPlanExecutionPlanMain() {
		return planExecutionPlanMainDao.getAll();
	}

	public void savePlanExecutionPlanMain(PlanExecutionPlanMainOld planExecutionPlanMainOld) {
		PowerUtils.setEmptyStr2Null(planExecutionPlanMainOld);
		planExecutionPlanMainDao.save(planExecutionPlanMainOld);
	}
	...
	@Override
	public HibernateDao<PlanExecutionPlanMainOld, String> getDao() {
	 ...
	 * @return
	 */
	public PlanExecutionPlanMainOld getPlanMainByProjCd(String projCd) {
		List<PlanExecutionPlanMainOld> list = this.getProjectListByProjectCd(projCd);
	 ...
	 * @return
	 */
	public PlanExecutionPlanMainOld getExecutionPlanMainByProjCd(String projectCd, String planTypeCd) {
		if (StringUtils.isBlank(projectCd))
			throw new IllegalArgumentException("项目CD不能为空");

		List<PlanExecutionPlanMainOld> results = this.getProjectListByProjectCd(projectCd);

		// 如果未建项目计划记录，则新建一条项目计划
		if (results == null || results.size() == 0) {
			PlanExecutionPlanMainOld pm = new PlanExecutionPlanMainOld();
		...
		//搜索指定类型的节点
		PlanExecutionPlanMainOld main = results.get(0);
		List<PlanProjectNodeRelOld> list = projNodeRelManager.getProjectNodeRelList(projectCd, planTypeCd);
		if(list == null || list.size() == 0){
	...
	// * @param pm
	// */
	// private void migrateNodeData(PlanExecutionPlanMainOld pm) {
	// List<PlanProjectNodeRelOld> projNodes = pm.getPlanProjectNodeRels();
	//
	// Map<String, PlanExecutionPlanNodeOld> nodes =
	// execPlanNodeManager.getExecPlanNodesMap();
	//
	// for (PlanProjectNodeRelOld nodeRel : projNodes) {
	// PlanExecutionPlanNodeOld n = nodes.get(nodeRel.getNodeCd());
	// if (n != null) {
	 ...
	 * 
	 */
	public void initProjectNodeRel(PlanExecutionPlanMainOld main, String planTypeCd, String projectCd) {
		if (main == null)
			throw new IllegalArgumentException("参数不能为null.");
		

		//List<PlanExecutionPlanNodeOld> allNodes = execPlanNodeManager.getAllPlanExecutionPlanNode();
		List<PlanExecutionPlanNodeOld> allNodes = execPlanNodeManager.getPlanExecutionNodeList(planTypeCd);
		String tmpCd = null;
		String tmpName = null;
		String projectName = CodeNameUtil.getDeptNameByCd(projectCd);
		for (PlanExecutionPlanNodeOld n : allNodes) {
			PlanProjectNodeRelOld r = new PlanProjectNodeRelOld();
	...
	public List<WsPlasOrg> fetchAllProjects() {
		...
		List<PlanExecutionPlanMainOld> allPlanMain = getAll();
		WsPlasOrg org = null;
	...
	public boolean saveReminders(String execPlanMainId, String reminders, String planTypeCd) throws Exception {
		...
		PlanExecutionPlanMainOld planMain = this.getEntity(execPlanMainId);
		...
		String hql = "update PlanExecutionPlanMainOld set " + fieldName
			...
			if (onlyCenCdQuery) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (!nowProject.equals(obj[6])) {
						...
						detailVO.setNodeName((String) obj[0]);
	...
	public List<Object> queryCostCtrlPlanRel(String nowDate, String nextMonthDate,boolean isExecPlan){
		...
		hql.append(" CostProjectNodeRel c,PlanExecutionPlanOld  d,CostExecutionPlanMain e");
		//过滤条件
	...
	private List<Object> queryCostPlanExecRel(String nowDate, String nextMonthDate,String centerCd){
		...
		hql.append(" from  PlanExecPlanDetailOld a,");
		hql.append(" PlanProjectNodeRelOld c,");
		hql.append(" PlanExecutionPlanOld d,");
		hql.append(" PlanExecutionPlanMainOld e,");
		hql.append(" CostExecutionPlanNode b,");
		hql.append(" PlanExecutionPlanNodeOld f");
	...
	public List<Object> queryCenterPlan(String nowDate, String nextMonthDate, String centerCd,
		...
		hql.append(" a.status, d.projectName,a.planExecPlanDetailId,b.planProjectNodeRelId,c.executionPlanCd,d.projectCd,c.planTypeCd");
		hql.append(" from  PlanExecPlanDetailOld a,");
		hql.append(" PlanProjectNodeRelOld b, ");
		hql.append("PlanExecutionPlanOld c,");
		hql.append("PlanExecutionPlanMainOld d ");
	...
	public List<PlanCenterDetailsVO> getProjectList(String centerCd, String planTypeCd) {
		...
		StringBuffer hql = new StringBuffer()
				...
				.append("   from PlanExecutionPlanMainOld a, PlanProjectNodeRelOld b ")
	 ...
	 * @return
	 */
	public List<PlanExecutionPlanMainOld> getProjectListByProjectCd(String projectCd) {

		List<PlanExecutionPlanMainOld> result = planExecutionPlanMainDao.createCriteria(PlanExecutionPlanMainOld.class)
											  .add(Restrictions.eq("projectCd", projectCd))
											  .list();
		if (result == null || result.size() == 0)
			return new ArrayList<PlanExecutionPlanMainOld>();
		return result;
	}
}