...
/**
 * 
 * 类名 CompanyStandardAction 创建者 lijin 创建日期 2010-4-12 描述 管理体系Action类
 */
@Namespace("/oa")
@Results({ @Result(name = CrudActionSupport.RELOAD, location = "company-file.action", type = "redirect") })
public class CompanyStandardAction extends CrudActionSupport<OaMeeting> {
	...
	public String loadFolderTree() throws Exception {
		String folderJSON = "";
		if (SpringSecurityUtils.hasRole("A_COMPANYFILE_ADMIN")) {
			folderJSON = oaFilingFolderManager.buildFolderTreeJSON(folderId,
					DictContants.COMPANY_FILING_MODULE_CD_STANDARD, null);
		} else {
			folderJSON = oaFilingFolderManager.buildFolderTreeJSON(folderId,
					DictContants.COMPANY_FILING_MODULE_CD_STANDARD, SpringSecurityUtils.getCurrentUiid());
		}
	...
	public String fileList() throws Exception {
		...
		Struts2Utils.getRequest().setAttribute("fileVoList", fileVoList);
		return "fileList";
	}
	...
	public String saveFolder() throws Exception {
		...
		folderEntity.setBizModuleCd(DictContants.COMPANY_FILING_MODULE_CD_STANDARD);
