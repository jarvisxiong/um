 ...
 *
 */
public class DealMaterialState extends BorderPage {
	...
	public String title = this.getMessage("dealmaterialstate");
	public String position = this.getMessage("dealMaterialState");
	public Menu menu = new Menu();
	public String menuInclude = menu.getMaterial();
	public DealMaterialState(){
		table.setWidth("100%");
	...
	public void onRender(){
		table.setRowList(mss.findAll());
	}
	
	public boolean onSubmit(){
		if(getContext().getSessionAttribute("dealMaterialStateId")!=null){
			String id = (String)getContext().getSessionAttribute("dealMaterialStateId");
			MaterialState materialState = mss.findMaterialState(id);
			materialState.setCode(code.getValue());
			materialState.setDescription(description.getValue());
			mss.updateMaterialState(materialState);
			form.clearValues();
		}else{
			MaterialState materialState = new MaterialState();
			materialState.setCode(code.getValue());
			materialState.setDescription(description.getValue());
			mss.saveMaterialState(materialState);
			form.clearValues();
		}
		getContext().removeSessionAttribute("dealMaterialStateId");
		return true;
	}
	
	public boolean onReset(){
		getContext().removeSessionAttribute("dealMaterialStateId");
		this.setRedirect("dealMaterialState.htm");
		return true;
	}
	
	public boolean onEdit(){
		String id = edit.getValue();
		MaterialState ms = mss.getMaterialState(id);
		form.copyFrom(ms);
		getContext().setSessionAttribute("dealMaterialStateId",id);
		return true;
	}
}
