...
public class StorageItemSearch extends com.intelitune.nwms.common.BorderPage {
	
	Menu me = new Menu();
	
	public String menuInclude = me.getSearch();
	
	public String position = this.getMessage("search_position");
	
	public String title=this.getMessage("storage_search");
...
//		});
		...
		table.addColumn(new Column("f_currency","币种"));
		...
		column_itemtype.setDecorator(new Decorator() {
			public String render(Object object, Context context) {
				StorageItem  si = (StorageItem) object;
				if(si.getItemType()==null){
					return "";
				}else{
					return si.getItemType().getAlias();
				}
			}
		 });
...
//		}
		for(int i=0;i<list.size();i++){
			if(list.get(i).getQty()!=0 && list.get(i).nextItems.size()==0){
	...
	private String generator(String owner_id,String warehouse_id) throws Exception{
		List<StorageItem> result1=sis.findStorageItemByWarehouseAndClient(owner_id, warehouse_id);
		List<StorageItem> result = new ArrayList();
		for(int i=0;i<result1.size();i++){			
			if(result1.get(i).getQty()!=0 && result1.get(i).getNextItems().size()==0){
